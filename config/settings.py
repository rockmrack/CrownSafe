...
class Settings(BaseSettings):
    ...
    def construct_database_url(cls, values):
        ...
        # Debug logging
        logging.info(f"[DEBUG] BEFORE: environment={environment}, is_production={is_production}, database_url={database_url}")
        logging.info(f"[DEBUG] DB_*: username={values.get('db_username')}, host={values.get('db_host')}, port={values.get('db_port')}, name={values.get('db_name')}")
