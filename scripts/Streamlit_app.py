...
class UnifiedMemoryManager:
    ...
    def _get_or_create_session_id(self):
        ...
        session_from_url = query_params.get("session", [None])[0] if isinstance(query_params.get("session", None), list) else query_params.get("session", None)
    ...
    def _update_conversation_summary(self):
        ...
        try:
            ...
            for msg in recent_messages:
                ...
                if any(word in content for word in ['decided', 'will use', 'architecture is', 'implemented']):
                    sentences = content.split('.')
                    for sentence in sentences:
                        if len(sentence) > 20 and any(word in sentence for word in ['decided', 'will use', 'architecture', 'implemented']):
...
Reference our previous discussions and maintain continuity.""",
                    messages=[{"role": "user", "content": context if attempt == 0 else self._request_completion(full_response, "Claude")}]
                )
...
{context}"""
    ...
    async def execute_gpt(self, prompt: str, attachments: List[Dict] = None) -> str:
        ...
        try:
            ...
            for attempt in range(max_retries):
                messages = [
                    {"role": "system", "content": """You are GPT-4, helping build CureViaX. You have access to our full conversation history.
...
- Production-ready implementations"""},
                    {"role": "user", "content": context if attempt == 0 else self._request_completion(full_response, "GPT")}
                ]
...
def export_conversation_to_pdf(messages: List[Message]) -> bytes:
    ...
    for msg in messages:
        if msg.role == "user":
            role_style = ParagraphStyle('UserStyle', parent=styles['Normal'], textColor='#2196f3', fontSize=12, spaceAfter=6)
            story.append(Paragraph(f"<b>You</b> - {msg.timestamp.strftime('%H:%M')}", role_style))
        else:
            role_style = ParagraphStyle('AssistantStyle', parent=styles['Normal'], textColor='#667eea', fontSize=12, spaceAfter=6)
            model_name = msg.model or "Assistant"
            story.append(Paragraph(f"<b>{model_name}</b> - {msg.timestamp.strftime('%H:%M')}", role_style))
        
        content_style = ParagraphStyle('ContentStyle', parent=styles['Normal'], fontSize=11, spaceAfter=20, leftIndent=20)
        content = msg.content.replace('```', '\n[CODE]\n')
        story.append(Paragraph(content[:10000], content_style))
        
        if msg.attachments:
            att_style = ParagraphStyle('AttachmentStyle', parent=styles['Normal'], fontSize=10, textColor='#666', italic=True, leftIndent=20)
...
with st.sidebar:
    ...
    with st.expander("Claude 4 Opus", expanded=False):
        claude_key = st.text_input("API Key", type="password", key="claude_key", 
    ...
    with st.expander("GPT-4", expanded=False):
        gpt_key = st.text_input("API Key", type="password", key="gpt_key",
...
# Header
memory_status = "Memory Active" if (st.session_state.connection_manager.status["redis"] or st.session_state.connection_manager.status["chromadb"]) else "Memory Inactive"
