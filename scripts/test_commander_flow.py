...
USER_REQUEST_PAYLOAD = {
    "goal": USER_RESEARCH_GOAL_STRING
}
...
async def run_test():
    ...
    try:
        ...
        logger.info(f"Status: {'COMPLETED' if monitor.workflow_complete else 'FAILED' if monitor.workflow_failed else 'TIMEOUT'}")
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(f"Fatal error in test_commander_flow.py main execution: {e_main}", exc_info=True)
