        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for member in members:
                    print(f"  - {member['name']}: Allergies: {', '.join(member['allergies']) if member['allergies'] else 'None'}")
            else:
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ Status: {data['status']}")
                print(f"Safety Level: {data['safety_level']} ({'🟢' if data['safety_level'] == 'SAFE' else '🔴' if data['safety_level'] == 'DANGER' else '🟡'})")
...
async def test_comprehensive_safety():
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            response = await client.post(
                f"{BASE_URL}/api/v1/premium/safety/comprehensive",
                json=request_data
            )
...
async def main():
    ...
    member_id = await test_family_members_endpoint()
