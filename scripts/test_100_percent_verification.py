...
def main():
    ...
    print("-" * 40)
    run_test("Safety Check", "POST", "/api/v1/safety-check", 
             {"barcode": "123456789012", "user_id": 1})
    run_test("Mobile Scan", "POST", "/api/v1/mobile/scan",
             {"user_id": 1, "barcode": "123456789012", "quick_scan": True})
    
    print("\n🔍 PREMIUM FEATURES")
    print("-" * 40)
    run_test("Pregnancy Safety", "POST", "/api/v1/premium/pregnancy/check",
             {"user_id": 1, "product": "Test Product", "trimester": 2, "barcode": "123456789"})
    run_test("Allergy Check", "POST", "/api/v1/premium/allergy/check",
             {"user_id": 1, "product": "Baby Food", "barcode": "123", "allergies": ["peanuts"]})
    run_test("Get Family Members", "GET", "/api/v1/premium/family/members", 
             params={"user_id": 1})
    run_test("Add Family Member", "POST", "/api/v1/premium/family/members",
             {"name": "Test Baby", "relationship": "child"},
             params={"user_id": 1}, expected=[200, 201])
    
    print("\n🔍 BABY SAFETY FEATURES")
    print("-" * 40)
    run_test("Safe Alternatives", "POST", "/api/v1/baby/alternatives",
             {"user_id": 1, "product": "Baby Lotion", "reason": "sensitive skin"})
    run_test("Push Notification", "POST", "/api/v1/baby/notifications/send",
             {"user_id": 1, "title": "Test Alert", "body": "Test message", "token": "test-token"})
    run_test("Community Alerts", "GET", "/api/v1/baby/community/alerts",
             params={"user_id": 1, "topic": "baby safety"})
    run_test("Hazard Analysis", "POST", "/api/v1/baby/hazards/analyze",
             {"user_id": 1, "product": "Baby Toy", "materials": ["plastic"]})
    
    print("\n🔍 ADVANCED FEATURES")
    print("-" * 40)
    run_test("Web Research", "POST", "/api/v1/advanced/research",
             {"user_id": 1, "product_name": "Baby Monitor", "research_depth": "comprehensive"})
    run_test("Age Guidelines", "POST", "/api/v1/advanced/guidelines",
    ...
    print("\n🔍 COMPLIANCE FEATURES")
    print("-" * 40)
    run_test("COPPA Age Verify", "POST", "/api/v1/compliance/coppa/verify-age",
             {"email": "parent@example.com", "birthdate": "1990-01-01", "parent_consent": True})
    run_test("Children's Code", "POST", "/api/v1/compliance/childrens-code/assess",
             {"user_id": 1, "age": 10, "country": "US", "features_used": ["safety_check", "alerts"], 
              "data_collected": ["NAME", "AGE"], "third_party_sharing": False})
    run_test("GDPR Data Request", "POST", "/api/v1/compliance/gdpr/data-request",
