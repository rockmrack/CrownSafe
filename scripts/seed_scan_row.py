...
values = dict(
    scan_id=scan_id, product_name=None, brand=None, manufacturer=None, model_number=None,
    barcode=barcode, upc_gtin=None, category=None, scan_type="barcode",
    confidence_score=1.0, barcode_format="EAN13", verdict="safe", risk_level="low",
    recalls_found=0, recall_ids="[]", agencies_checked=39, allergen_alerts="[]",
    pregnancy_warnings="[]", age_warnings="[]", included_in_reports=0,
...
if row:
    ...
    cols = ",".join(values.keys()); qs=",".join(["?"]*len(values))
