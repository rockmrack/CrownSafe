...
class DrugClassPatternTester:
    ...
    def __init__(self):
        ...
        self.sglt2_drugs = ["canagliflozin", "dapagliflozin", "empagliflozin", "ertugliflozin", "sotagliflozin"]
        ...
        self.expected_patterns = {
            ...
            "common_studies": ["Class effect studies", "Comparative effectiveness research", "Meta-analyses"]
        }
                ...
                try:
                    ...
                    if enhanced_analytics:
                        ...
                        if isinstance(enhanced_analytics, dict):
                            ...
                            insights = enhanced_analytics.get('insights', enhanced_analytics.get('cross_workflow_insights', 0))
                        else:
                            # If it's not a dict, try to get counts from the object
                            temporal = getattr(enhanced_analytics, 'temporal_patterns', 0) if hasattr(enhanced_analytics, 'temporal_patterns') else 0
                            contradictions = getattr(enhanced_analytics, 'contradictions', 0) if hasattr(enhanced_analytics, 'contradictions') else 0
                            gaps = getattr(enhanced_analytics, 'gaps', 0) if hasattr(enhanced_analytics, 'gaps') else 0
                            insights = getattr(enhanced_analytics, 'insights', 0) if hasattr(enhanced_analytics, 'insights') else 0
    ...
    async def _get_sglt2_cross_workflow_evidence(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = self.memory.collection.query(
                query_texts=["SGLT2 inhibitor canagliflozin empagliflozin sotagliflozin dapagliflozin cardiovascular outcomes"],
                n_results=50,
            ...
            if ENHANCED_AVAILABLE and hasattr(self.memory, 'get_enhanced_research_recommendations'):
                ...
                try:
                    ...
                    if recommendations:
                        ...
                        strategy = recommendations.get('research_strategy', 
                                  recommendations.get('strategy', 
                                  recommendations.get('recommended_strategy', 'unknown')))
                        
                        gaps = recommendations.get('knowledge_gaps', 
                               recommendations.get('gaps', 
                               recommendations.get('research_gaps', [])))
                        
                        priorities = recommendations.get('priority_areas', 
                                    recommendations.get('priorities', 
                                    recommendations.get('priority_research', [])))
                        
                        existing_evidence = recommendations.get('existing_evidence', 
                                          recommendations.get('existing_evidence_summary', 
                        ...
                        self.print_info(f"  Priority areas: {len(priorities) if isinstance(priorities, list) else priorities}")
                        self.print_info(f"  Related documents found: {len(related_docs) if isinstance(related_docs, list) else related_docs}")
                        self.print_info(f"  Similar drugs identified: {similar_drugs}")
                        self.print_info(f"  Cross-workflow opportunities: {len(cross_workflow_ops) if isinstance(cross_workflow_ops, list) else cross_workflow_ops}")
                        ...
                        if strategy in ['focused', 'update'] and (total_existing > 0 or len(similar_drugs) > 0 or len(cross_workflow_ops) > 0):
                            ...
                            self.print_info("  [CHECK] This suggests some class recognition even without explicit evidence count")
                        elif strategy == 'comprehensive' and total_existing > 0:
                            self.print_warning("MIXED: System found existing evidence but still recommends comprehensive strategy")
                            ...
                            self.print_info("  [X] This suggests get_enhanced_research_recommendations may have failed internally")
            ...
            for i, query in enumerate(queries):
                ...
                if results and results['metadatas'] and results['metadatas'][0]:
                    for j, (metadata, distance) in enumerate(zip(results['metadatas'][0], results['distances'][0])):
                        ...
                        self.print_debug(f"    Result {j+1}: distance={distance:.3f}, source={source}, drugs={drug_context}")
                else:
    ...
    async def _manual_ertugliflozin_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self.memory.collection.query(
                query_texts=["SGLT2 inhibitor Ertugliflozin heart failure cardiovascular outcomes sodium glucose cotransporter"],
                n_results=20,
            ...
            if results and results['metadatas'] and results['metadatas'][0]:
                ...
                for i, metadata in enumerate(metadatas):
                    ...
                    if i < 5:
                        ...
                        self.print_info(f"  Result {i+1} (distance: {distance:.3f}): {metadata.get('source_type', 'unknown')}")
    ...
    async def test_cross_drug_evidence_retrieval(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self.memory.collection.query(
                query_texts=["SGLT2 inhibitors cardiovascular outcomes heart failure class effect meta-analysis comparative effectiveness"],
                n_results=30,
                include=["metadatas", "documents"]
            )
            ...
            self.print_success(f"SGLT2 drug diversity: {len(unique_sglt2_drugs)} unique drugs: {sorted(list(unique_sglt2_drugs))}")
            ...
            comp_results = self.memory.collection.query(
                query_texts=["Canagliflozin Dapagliflozin Empagliflozin Sotagliflozin comparative effectiveness cardiovascular outcomes head to head"],
                n_results=10,
                include=["metadatas"]
            )
            ...
            if comp_results and comp_results['metadatas'] and comp_results['metadatas'][0]:
                ...
                if comp_examples:
                    ...
                    for i, example in enumerate(comp_examples[:3]):
                        self.print_info(f"  {i+1}. {example['id']}... ({example['source']}): compares {example['drugs']}")
        ...
        try:
            ...
            for i, metadata in enumerate(all_docs['metadatas']):
                ...
                if is_sglt2:
                        ...
                        if isinstance(drug, str) and drug.startswith('[') and drug.endswith(']'):
                            try:
                                ...
                                for parsed_drug in parsed_drugs:
                                    sglt2_stats["by_drug"][str(parsed_drug)] = sglt2_stats["by_drug"].get(str(parsed_drug), 0) + 1
                            except:
                    ...
                    if timestamp:
                        if not sglt2_stats["temporal_spread"]["oldest"] or timestamp < sglt2_stats["temporal_spread"]["oldest"]:
                            sglt2_stats["temporal_spread"]["oldest"] = timestamp
                        if not sglt2_stats["temporal_spread"]["newest"] or timestamp > sglt2_stats["temporal_spread"]["newest"]:
            ...
            self.print_info(f"  Total SGLT2 documents: {sglt2_stats['total_sglt2_docs']}/{total_docs} ({sglt2_stats['total_sglt2_docs']/total_docs*100:.1f}%)")
            ...
            if sglt2_stats["drug_combinations"]:
                ...
                for combo, count in sorted(sglt2_stats["drug_combinations"].items(), 
                    ...
                    combo_str = ' + '.join(clean_combo) if len(clean_combo) <= 3 else f"{clean_combo[0]} + {clean_combo[1]} + ... ({len(clean_combo)} drugs)"
    ...
    async def generate_test_summary(self, current_patterns: Dict[str, Any], 
                                  ertugliflozin_sim: Dict[str, Any],
                                  cross_drug_results: Dict[str, Any],
                                  sglt2_kb: Dict[str, Any]) -> None:
        ...
        effective_evidence_count = max(sglt2_evidence_count, cross_workflow_count if cross_workflow_count > 0 else 0)
        ...
        predicted_strategy = ertugliflozin_sim.get('research_strategy', 
        ...
        if predicted_strategy == 'update' and sglt2_related >= 20:
            ...
            self.print_error(f"CRITICAL: get_enhanced_research_recommendations returned '{predicted_strategy if predicted_strategy != 'update' else 'unknown'}' strategy")
        ...
        self.print_info("  1. Raw response shows all empty arrays: {'priority_research': [], 'gap_addressing': [], ...}")
