...
try:
    ...
    if os.path.exists(dotenv_path):
        try:
            ...
            print(
                "[RunAgent] Warning: 'python-dotenv' not installed. Cannot load .env file explicitly. Relying on automatic loading by libraries."
            )
        except Exception as load_e:
            print(f"[RunAgent] Error loading .env file: {load_e}")
    else:
        print(
            f"[RunAgent] Note: .env file not found at {dotenv_path}, environment variables should be set manually if needed."
        )
