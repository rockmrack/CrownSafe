...
async def test_high_confidence_visual_workflow():
    ...
    try:
        ...
        assert result["status"] in ["COMPLETED", "INCONCLUSIVE"], f"Expected completion, got {result['status']}"
                        ...
                        if "top 3" in prompt:
                            ...
                            class Response:
                                class Choice:
                                    class Message:
                                        content = json.dumps([
                                            {"product_name": "Product A", "brand": "Brand A", "model_number": "A1", "confidence": 0.9},
                                            {"product_name": "Product B", "brand": "Brand B", "model_number": "B1", "confidence": 0.7},
                                            {"product_name": "Product C", "brand": "Brand C", "model_number": "C1", "confidence": 0.5}
                                        ])
                                    message = Message()
                                choices = [Choice()]
                            return Response()
