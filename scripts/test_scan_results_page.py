...
def test_scan_results_page():
    ...
    assert result_safe.sub_text == "No recalls or allergen issues found in our database.", "Sub-text should clarify database check"
    assert "39+ (No recalls found)" in result_safe.safety_check.agencies_checked, "Should show 39+ agencies"
    assert result_safe.barcode_detection.quality_badge == "Best Quality: 014292998228 (ean13)", "Should show quality badge"
    ...
    assert "39+ (2 recalls found)" in result_recall.safety_check.agencies_checked, "Should show recall count in agencies"
