...
from agents.recall_data_agent.connectors import (
    CPSCConnector, FDAConnector, NHTSAConnector, EURAPEXConnector
)
        ...
        try:
            with get_db_session() as db:
                ...
                if batch_records:
                    ...
                    logger.info(f"📊 {agency_name}: {counts['inserted']} inserted, {counts['updated']} updated, {counts['failed']} failed")
...
async def main():
    ...
    parser.add_argument("--migrate-only", action="store_true", help="Only run migration, don't ingest data")
