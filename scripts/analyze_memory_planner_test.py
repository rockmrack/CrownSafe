...
class MemoryPlannerAnalyzer:
    ...
    def _score_log_candidate(self, candidate: LogCandidate, drug_name: str = None, 
                           workflow_id: str = None, reference_time: float = None) -> float:
    ...
    def _analyze_log_content(self, log_path: Path, drug_name: str = None, 
                           workflow_id: str = None, check_depth: int = 50000) -> LogCandidate:
        """Analyze log content and create a scored candidate"""
        candidate = LogCandidate(
            path=log_path,
            modification_time=log_path.stat().st_mtime
        )
    ...
    def _find_best_log_file(self, agent_type: str, drug_name: str = None, workflow_id: str = None, 
                           specific_log_path: str = None, time_window_minutes: int = 60) -> Optional[Path]:
        ...
        if drug_name:
            ...
            if pdf_info:
                ...
                print(f"[INFO] Using PDF timestamp as reference: {datetime.fromtimestamp(pdf_time).strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for i, candidate in enumerate(candidates[:3]):
            ...
            print(f"     - Modified: {datetime.fromtimestamp(candidate.modification_time).strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def analyze_planner_logs(self, workflow_id: str = None, drug_name: str = None, 
                           planner_log_path: str = None) -> Dict[str, Any]:
                ...
                for pattern in entity_patterns:
                    ...
                    for match in matches:
                        try:
                            ...
                            extracted_disease = extracted_entities.get('primary_disease') or extracted_entities.get('disease_name')
                            ...
                            if drug_name and extracted_drug:
                                if extracted_drug.lower() == drug_name.lower():
                                    ...
                                    print(f"[OK] Found matching entity extraction: Drug={extracted_drug}, Disease={extracted_disease}")
                                    entity_found = True
                                    break
                                else:
                                    print(f"[INFO] Found entity extraction for different drug: {extracted_drug} (looking for {drug_name})")
                            elif not drug_name:
                        ...
                        try:
                            ...
                            if drug_name and plan_drug:
                                if plan_drug.lower() != drug_name.lower():
                                    print(f"[INFO] Found plan for different drug: {plan_drug} (looking for {drug_name})")
                                    continue
                            ...
                            print(f"[OK] Plan structure analyzed: {analysis['plan_structure']['steps_count']} steps, strategy={analysis['plan_structure']['research_strategy']}")
                            plan_found = True
                            break
        ...
        if drug_name and analysis.get("entities_extracted"):
            extracted_drug = analysis["entities_extracted"].get("primary_drug") or analysis["entities_extracted"].get("drug_name")
            if extracted_drug and extracted_drug.lower() != drug_name.lower():
                ...
                print(f"  - This log may be from a different workflow!")
        
        self.analysis_results["planner_performance"] = analysis
        return analysis
    
    def analyze_workflow_execution(self, workflow_id: str = None, drug_name: str = None,
                                 router_log_path: str = None, commander_log_path: str = None) -> Dict[str, Any]:
        ...
        worker_configs = [
            ("pubmed_search_agent", "web_research_agent", [
            ...
            ("clinical_trials_agent", "clinical_trials", [
                r"Found (\d+) trials", 
                r"Retrieved (\d+) trials", 
                r"trials found: (\d+)",
                r"Total trials: (\d+)",
                r"Retrieved (\d+) clinical trials",
                r"Found (\d+) clinical trials"
            ], "clinical_trials", "clinical_trials"),
            ("drug_safety_agent", "drug_safety", [
                r"Retrieved (\d+) adverse events", 
                r"Found (\d+) adverse events", 
                r"events found: (\d+)",
                r"Total events: (\d+)",
                r"top_adverse_reactions.*?(\d+)",
                r"Found (\d+) unique adverse reactions"
            ], "adverse_events", "fda")
        ]
        
        for primary_name, alt_name, patterns, data_key, api_key in worker_configs:
            worker_log = self._find_best_log_file(primary_name, drug_name, workflow_id) or \
                        self._find_best_log_file(alt_name, drug_name, workflow_id)
    ...
    async def analyze_memory_impact(self, drug_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if drug_docs["metadatas"]:
                ...
                analysis["new_documents"] = len([m for m in drug_docs["metadatas"] 
                                               if len(m.get("referenced_in_workflows", [])) == 1])
            ...
            if planner_strategy:
                ...
                sglt2_drugs = ["canagliflozin", "dapagliflozin", "empagliflozin", "ertugliflozin", "sotagliflozin"]
                ...
                if is_sglt2:
                    ...
                    analysis["strategy_validation"] = {
                        ...
                        "strategy_appropriate": (
                            planner_strategy == "comprehensive" if sglt2_count < 20 else
                            planner_strategy in ["focused", "update"]
                        )
                    }
    ...
    def generate_recommendations(self, drug_name: str) -> List[str]:
        ...
        if not planner:
            recommendations.append(f"No planner logs found - ensure logs are retained and check log directory: {self.logs_dir}")
        elif planner.get("fallback_used"):
            recommendations.append("Consider improving GPT-4 prompt reliability or implementing better fallback handling")
        ...
        if planner.get("entities_extracted", {}).get("primary_drug"):
            ...
            if drug_name and extracted_drug.lower() != drug_name.lower():
                recommendations.append(f"[CRITICAL] Analysis may be for wrong drug: extracted '{extracted_drug}' but expected '{drug_name}'")
                recommendations.append(f"[ACTION] Verify log selection or use --planner-log to specify correct log file")
        ...
        if planner_metadata:
            ...
            if score < 30:
                recommendations.append(f"[WARNING] Low confidence in log selection (score: {score:.1f}). Consider using --planner-log parameter")
        ...
        elif drug_name and ("sglt2" in drug_name.lower() or drug_name.lower() in ["canagliflozin", "dapagliflozin", "empagliflozin", "ertugliflozin", "sotagliflozin"]):
            total_docs = memory_util.get("strategy_validation", {}).get("class_documents", 0)
            if total_docs > 30 and strategy == "comprehensive":
                recommendations.append(f"Consider using 'focused' or 'update' strategy for SGLT2 inhibitors (already have {total_docs} docs)")
            elif strategy in ["focused", "update"]:
                recommendations.append(f"[OK] Good strategy choice! Using '{strategy}' strategy for well-studied drug class")
        ...
        if drug_name:
            if drug_name.lower() == "empagliflozin":
                ...
                recommendations.append("Next: Test a different drug class (e.g., Lisinopril) - should trigger 'comprehensive' strategy")
    ...
    async def run_complete_analysis(self, workflow_id: Optional[str] = None, drug_name: str = "Empagliflozin",
                                  planner_log: str = None, router_log: str = None, commander_log: str = None):
        """Run complete analysis of memory-augmented planner test"""
        print("\n" + "="*60)
        print(f"MEMORY-AUGMENTED PLANNER TEST ANALYSIS v4.0")
        print("="*60)
        print(f"Target Drug: {drug_name}")
        ...
        memory_augmented = self.analysis_results["planner_performance"].get("plan_structure", {}).get("memory_augmented", False)
        ...
        print(f"  - Total Data Retrieved: {sum(self.analysis_results['workflow_efficiency'].get('data_retrieved', {}).values())}")
        ...
        print(f"\n[DISK] MEMORY IMPACT:")
        print(f"  - Total {drug_name} Documents: {self.analysis_results['memory_utilization'].get('total_drug_documents', 0)}")
        ...
        print(f"  - Cross-Drug Evidence: {len(self.analysis_results['memory_utilization'].get('cross_drug_evidence', []))}")
        print(f"  - High-Quality Docs: {self.analysis_results['memory_utilization'].get('quality_distribution', {}).get('high', 0)}")
...
Examples:
  ...
  python analyze_memory_planner_test.py -d Canagliflozin --planner-log "C:\\logs\\planner_agent_Cana.log"
    ...
    # Run analysis
    asyncio.run(analyzer.run_complete_analysis(
