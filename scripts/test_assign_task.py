    ...
    if correlation_id == task_correlation_id:
        if message_type in ["TASK_COMPLETE", "TASK_FAIL"]:
            ...
            task_completion_event.set() # Signal that the task is done
             ...
             logger.info(f"Received TASK_ACKNOWLEDGE for correlation_id: {correlation_id}, Status: {payload.get('status')}")
        elif message_type == "STATUS_UPDATE":
             logger.info(f"Received STATUS_UPDATE for correlation_id: {correlation_id}, Description: {payload.get('status_description')}")
        else:
...
async def run_task_assignment():
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred in the task assignment test: {e}", exc_info=True)
    finally:
