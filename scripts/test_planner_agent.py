...
def test_template_based_planning(logic: MemoryAugmentedPlannerLogic):
    ...
    # Step 1 - patient data
    step1 = plan['steps'][0]
    assert step1['inputs']['patient_id'] == 'patient-001', f"Step 1: Expected patient_id 'patient-001', got {step1['inputs'].get('patient_id')}"
    
    # Step 2 - guidelines
    step2 = plan['steps'][1]
    expected_topic = "Guidelines for Empagliflozin in treating Heart Failure"
    assert step2['inputs']['topic'] == expected_topic, f"Step 2: Expected topic '{expected_topic}', got {step2['inputs'].get('topic')}"
    
    # Step 3 - policy
    step3 = plan['steps'][2]
    assert step3['inputs']['drug_name'] == 'Empagliflozin', f"Step 3: Expected drug_name 'Empagliflozin', got {step3['inputs'].get('drug_name')}"
    assert step3['inputs']['insurer_id'] == 'UHC-123', f"Step 3: Expected insurer_id 'UHC-123', got {step3['inputs'].get('insurer_id')}"
    
    # Step 4 - final prediction
    final_step = plan['steps'][3]
    assert final_step['step_id'] == 'step4_predict_approval'
    assert final_step['inputs']['patient_id'] == 'patient-001', f"Step 4: Expected patient_id 'patient-001', got {final_step['inputs'].get('patient_id')}"
    assert final_step['inputs']['drug_name'] == 'Empagliflozin', f"Step 4: Expected drug_name 'Empagliflozin', got {final_step['inputs'].get('drug_name')}"
    assert final_step['inputs']['insurer_id'] == 'UHC-123', f"Step 4: Expected insurer_id 'UHC-123', got {final_step['inputs'].get('insurer_id')}"
...
def test_plan_validation(logic: MemoryAugmentedPlannerLogic):
    ...
    required_step_fields = ['step_id', 'task_description', 'agent_capability_required', 
                           'inputs', 'dependencies', 'priority']
    ...
    valid_capabilities = ['perform_web_search', 'retrieve_clinical_trials', 
                         'query_adverse_events', 'build_final_report']
...
def run_all_tests():
    ...
    logic = MemoryAugmentedPlannerLogic(
        agent_id="test_planner_agent_01",
    ...
    for test_name, test_func in test_suites:
        try:
            ...
            logger.error(f"Test failure details:", exc_info=True)
            failed_tests += 1
