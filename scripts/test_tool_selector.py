...
async def handle_controller_message(message: Dict[str, Any]):
    ...
    if correlation_id == task_correlation_id:
        if message_type in ["TASK_COMPLETE", "TASK_FAIL"]:
            logger.info(
                f"Received final TOOL_SELECTOR task status: {message_type} for correlation_id: {correlation_id}"
            )
...
async def run_tool_selection():
    ...
    try:
        ...
        try:
            ...
            if task_result:
                ...
                if task_result.get("mcp_header", {}).get("message_type") == "TASK_COMPLETE":
                    ...
                    if output_data.get("status") == "DELEGATED":
                        ...
                        logger.warning(
                            f"ToolSelector completed but did not delegate successfully: Status={output_data.get('status')}"
                        )
                else:
            ...
            logger.error(
                f"Timeout: Did not receive tool selector task completion message within {TASK_TIMEOUT_SECONDS} seconds."
            )
