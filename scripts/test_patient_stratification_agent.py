...
from agents.patient_stratification_agent.agent_logic import PatientStratificationAgentLogic, DecisionType
...
async def test_stratification_agent():
        ...
        if result['status'] == 'success':
            ...
            for evidence in prediction['evidence_items']:
                if evidence['supports_approval']:
                    print(f"   • [{evidence['source']}] {evidence['content']} (confidence: {evidence['confidence']:.0%})")
            ...
            assert normalized_decision == DecisionType.APPROVE.value, f"Expected {DecisionType.APPROVE.value}, got {normalized_decision}"
            assert prediction['approval_likelihood'] > 70, f"Expected high likelihood, got {prediction['approval_likelihood']}%"
        ...
        if result['status'] == 'success':
            ...
            if prediction.get('alternative_options'):
                ...
                for alt in prediction['alternative_options']:
                    print(f"   • {alt['drug_name']} - {alt.get('coverage_status', 'Unknown')} (Tier {alt.get('tier', 'N/A')})")
            
            # Validate expected outcome
            assert normalized_decision == DecisionType.DENY.value, f"Expected {DecisionType.DENY.value}, got {normalized_decision}"
            assert prediction['approval_likelihood'] < 30, f"Expected low likelihood, got {prediction['approval_likelihood']}%"
    ...
    try:
        ...
        if result['status'] == 'success':
            ...
            assert normalized_decision in [DecisionType.DENY.value, DecisionType.PEND.value], f"Expected DENY or PEND, got {normalized_decision}"
    ...
    try:
        ...
        if result['status'] == 'success':
            ...
            urgency_evidence = [e for e in evidence_items if 'urgency' in e.get('type', '').lower() or 'urgent' in e.get('content', '').lower()]
...
async def test_error_handling():
    ...
    result = await logic.predict_approval_likelihood(
        patient_id="invalid-patient-999",
        drug_name="Empagliflozin",
        insurer_id="UHC"
    )
