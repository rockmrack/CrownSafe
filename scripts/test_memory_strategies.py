...
class MemoryStrategyTester:
    ...
    def _extract_strategy_from_logs(self) -> Optional[str]:
        ...
        try:
            ...
            planner_logs = [f for f in os.listdir(self.logs_dir) if f.startswith("planner_agent_") and f.endswith(".log")]
            if not planner_logs:
                return None
            
            latest_log = max(planner_logs, key=lambda f: os.path.getmtime(os.path.join(self.logs_dir, f)))
    ...
    def _extract_workflow_id(self) -> Optional[str]:
        ...
        try:
            commander_logs = [f for f in os.listdir(self.logs_dir) if f.startswith("commander_agent_") and f.endswith(".log")]
            if not commander_logs:
                return None
            
            latest_log = max(commander_logs, key=lambda f: os.path.getmtime(os.path.join(self.logs_dir, f)))
            log_path = os.path.join(self.logs_dir, latest_log)
