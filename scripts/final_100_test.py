...
# Premium
test("Pregnancy", "POST", "/api/v1/premium/pregnancy/check", {"user_id": 1, "product": "Test", "trimester": 2, "barcode": "123"})
test("Allergy", "POST", "/api/v1/premium/allergy/check", {"user_id": 1, "product": "Food", "barcode": "123", "allergies": ["nuts"]})
...
r = requests.post(f"{BASE_URL}/api/v1/premium/family/members?user_id=1", json={"name": unique_name, "relationship": "child"})
...
test("Notification", "POST", "/api/v1/baby/notifications/send", {"user_id": 1, "title": "Alert", "body": "Test", "token": "test"})
test("Community", "GET", "/api/v1/baby/community/alerts", params={"user_id": 1, "topic": "safety"})
test("Hazard", "POST", "/api/v1/baby/hazards/analyze", {"user_id": 1, "product": "Toy", "materials": ["plastic"]})

# Advanced
test("Research", "POST", "/api/v1/advanced/research", {"user_id": 1, "product_name": "Monitor"})
test("Guidelines", "POST", "/api/v1/advanced/guidelines", {"user_id": 1, "child_age_months": 6, "product_category": "toys"})
print("  Visual Recognition (file upload endpoint)")

# Compliance
test("COPPA", "POST", "/api/v1/compliance/coppa/verify-age", {"email": "test@test.com", "birthdate": "1990-01-01"})
test("Children Code", "POST", "/api/v1/compliance/childrens-code/assess", 
     {"user_id": 1, "age": 10, "country": "US", "features_used": ["safety"], "data_collected": ["NAME"], "third_party_sharing": False})
test("GDPR", "POST", "/api/v1/compliance/gdpr/data-request", {"user_id": 1, "request_type": "access", "email": "user@test.com"})
