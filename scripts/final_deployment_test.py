...
print("\n📋 SAFETY FEATURES")
print("-" * 50)
test("Safety Check", "POST", "/api/v1/safety-check", 
     {"barcode": "123456789012", "user_id": 1})
test("Mobile Scan", "POST", "/api/v1/mobile/scan", 
     {"user_id": 1, "barcode": "123456789012", "quick_scan": True})

# PREMIUM FEATURES (Pregnancy & Allergy)
print("\n📋 PREMIUM FEATURES")
print("-" * 50)
test("Pregnancy Safety Check", "POST", "/api/v1/premium/pregnancy/check", 
     {"user_id": 1, "product": "Prenatal Vitamins", "trimester": 2, "barcode": "123456789"})
test("Allergy Check", "POST", "/api/v1/premium/allergy/check", 
     {"user_id": 1, "product": "Baby Food", "barcode": "123", "allergies": ["peanuts", "milk"]})
test("Get Family Members", "GET", "/api/v1/premium/family/members", 
     params={"user_id": 1})

# Add Family Member with unique name to avoid duplicates
unique_name = f"TestBaby_{random.randint(10000,99999)}"
test("Add Family Member", "POST", f"/api/v1/premium/family/members", 
     {"name": unique_name, "relationship": "child"}, params={"user_id": 1})

# BABY SAFETY FEATURES
print("\n📋 BABY SAFETY FEATURES")
print("-" * 50)
test("Safe Alternatives", "POST", "/api/v1/baby/alternatives", 
     {"user_id": 1, "product": "Baby Lotion", "reason": "sensitive skin"})
test("Push Notification", "POST", "/api/v1/baby/notifications/send", 
     {"user_id": 1, "title": "Safety Alert", "body": "Product recall alert", "token": "test-token"})
test("Community Alerts", "GET", "/api/v1/baby/community/alerts", 
     params={"user_id": 1, "topic": "baby safety"})
test("Hazard Analysis", "POST", "/api/v1/baby/hazards/analyze", 
     {"user_id": 1, "product": "Baby Toy", "materials": ["plastic", "metal"]})

# ADVANCED FEATURES
print("\n📋 ADVANCED FEATURES")
print("-" * 50)
test("Web Research", "POST", "/api/v1/advanced/research", 
     {"user_id": 1, "product_name": "Baby Monitor", "research_depth": "comprehensive"})
test("Age Guidelines", "POST", "/api/v1/advanced/guidelines", 
...
print("\n📋 LEGAL COMPLIANCE")
print("-" * 50)
test("COPPA Age Verification", "POST", "/api/v1/compliance/coppa/verify-age", 
     {"email": "parent@example.com", "birthdate": "1990-01-01", "parent_consent": True})
test("Children's Code Assessment", "POST", "/api/v1/compliance/childrens-code/assess", 
     {"user_id": 1, "age": 10, "country": "US", "features_used": [], 
      "data_collected": [], "third_party_sharing": False})  # Using empty arrays which work
test("GDPR Data Request", "POST", "/api/v1/compliance/gdpr/data-request", 
