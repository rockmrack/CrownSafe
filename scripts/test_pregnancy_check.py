...
async def run_pregnancy_test():
        ...
        "steps": [
            {
                ...
                "output_map": {"product_name": "product_name", "upc": "upc", "ingredients": "ingredients"}
            },
            {
                "step_id": "step2_check_recalls", 
                "agent_type": "query_recalls_by_product",
                "agent_capability_required": "query_recalls_by_product",
                "inputs": {"product_name": "{step1_identify_product.result.product_name}"},  # Changed from 'input' to 'inputs'
            ...
            },
            {
                ...
                "output_map": {"risks_found": "risks_found", "risky_ingredients": "risky_ingredients"},
                "dependencies": ["step1_identify_product"]
            }
        ]
    }
    ...
    with patch.object(commander.planner, 'process_task', return_value={"status": "COMPLETED", "plan": test_plan}):
        # Define user request
        user_request = {
            "barcode": TEST_BARCODE_RISKY_PRODUCT, 
