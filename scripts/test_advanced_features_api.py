    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Safety Score: {data['safety_score']}/100 {'🟢' if data['safety_score'] > 80 else '🟡' if data['safety_score'] > 60 else '🔴'}")
                ...
                if data['findings']:
                    ...
                    for finding in data['findings'][:3]:
                        sentiment_emoji = "😊" if finding['sentiment'] == "positive" else "😟" if finding['sentiment'] == "negative" else "😐"
                ...
                if response.status_code == 200:
                    ...
                    if data['guidelines']:
                        ...
                        for guideline in data['guidelines'][:2]:
                            importance_emoji = "🔴" if guideline['importance'] == "critical" else "🟡" if guideline['importance'] == "important" else "🟢"
...
async def test_visual_recognition():
    ...
    if not mock_image_path.exists():
        ...
        mock_jpeg = bytes([
            ...
            0x49, 0x46, 0x00, 0x01,  # JFIF version
            0x01, 0x01, 0x00, 0x00,  # Aspect ratio
            0x00, 0x00, 0x00, 0x00,  # Thumbnail
...
async def test_monitoring():
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            response = await client.post(
                f"{BASE_URL}/api/v1/advanced/monitor/setup",
                json=request_data
            )
