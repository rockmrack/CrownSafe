...
if PROM:
    CHAT_REQ = Counter("bs_chat_requests_total", "Chat requests", ["endpoint","intent","ok","circuit"])
    CHAT_LAT = Histogram("bs_chat_total_latency_ms", "Total latency (ms)", buckets=(50,100,200,400,800,1600,3200,6400))
    TOOL_LAT = Histogram("bs_chat_tool_latency_ms", "Tool latency (ms)", ["intent"], buckets=(10,50,100,200,400,800,1600))
    SYN_LAT  = Histogram("bs_chat_synth_latency_ms", "Synth latency (ms)", buckets=(50,100,200,400,800,1600))
    ...
    CHAT_REQ = CHAT_LAT = TOOL_LAT = SYN_LAT = CHAT_FALLBACK = CHAT_BLOCKED = EXPLAIN_FB = ALT_SHOWN = ALT_CLICKED = CHAT_UNCLEAR = CHAT_EMERG = _N()
