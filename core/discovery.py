...
def _normalize_capability_name(cap_name: str) -> str:
    ...
    if not isinstance(cap_name, str):
        discovery_logger.warning(f"Invalid capability name (not a string): {cap_name} (type: {type(cap_name)}). Attempting conversion.")
...
def _get_agent_capabilities(agent_info: Dict[str, Any]) -> Set[str]:
    ...
    if not isinstance(agent_info, dict):
        discovery_logger.warning(f"_get_agent_capabilities: agent_info is not a dict. Got: {type(agent_info)}")
        return capabilities
        
    agent_caps_list = agent_info.get("capabilities", [])
    agent_id_for_log = agent_info.get('agent_id', 'UnknownAgent')
    discovery_logger.debug(f"_get_agent_capabilities for '{agent_id_for_log}': Raw capabilities list: {agent_caps_list}")
        ...
        for cap_item in agent_caps_list:
            if isinstance(cap_item, dict) and "name" in cap_item:
                ...
                if isinstance(cap_name_val, str):
                    ...
                    discovery_logger.debug(f"  Added capability from dict for '{agent_id_for_log}': '{cap_name_val}' -> '{normalized_cap}'")
                else:
                    discovery_logger.warning(f"  Invalid capability name type in dict for '{agent_id_for_log}': {type(cap_name_val)}")
            elif isinstance(cap_item, str):
                normalized_cap = _normalize_capability_name(cap_item)
                capabilities.add(normalized_cap)
                discovery_logger.debug(f"  Added capability from string for '{agent_id_for_log}': '{cap_item}' -> '{normalized_cap}'")
            else:
                discovery_logger.warning(f"  Invalid capability item format for '{agent_id_for_log}': {cap_item} (type: {type(cap_item)})")
    elif agent_caps_list: 
        discovery_logger.warning(f"  Invalid capabilities format for '{agent_id_for_log}': Expected list, got {type(agent_caps_list)}. Value: {agent_caps_list}")
...
async def handle_discovery_message(message: Dict[str, Any], sender_id: str):
    ...
    if not message or not isinstance(message, dict):
        discovery_logger.error(f"Invalid message format received from '{sender_id}': Not a dict or empty. Type: {type(message)}")
        return 
    ...
    discovery_logger.info(f"Discovery received '{message_type}' from '{sender_id}' (CorrID: {correlation_id})")
        ...
        for log_key, log_value in payload.items():  # Using very distinct loop variable names
            if log_key == "capabilities" and isinstance(log_value, list):
                ...
                try:
                    ...
                    temp_log_payload_dict[log_key] = val_str[:100] + "..." if len(val_str) > 100 else val_str
                except Exception:
    ...
    try:
        if message_type == "DISCOVERY_REGISTER":
            ...
            discovery_logger.warning(f"Unknown message type '{message_type}' from '{sender_id}' (CorrID: {correlation_id}).")
            error_payload = {"error_code": "E003", "error_message": f"Unsupported message type '{message_type}'"}
            response_message = create_mcp_error_response(sender_id, correlation_id, "MCP_DISCOVERY", error_payload)
    except Exception as e:
        discovery_logger.error(f"Error processing message type '{message_type}' from '{sender_id}' (CorrID: {correlation_id}): {e}", exc_info=True)
        error_payload = {"error_code": "E999", "error_message": f"Internal error processing message: {str(e)}"}
        response_message = create_mcp_error_response(sender_id, correlation_id, "MCP_DISCOVERY", error_payload)
            ...
            try:
                if "payload" in response_message and isinstance(response_message["payload"], dict):
                    ...
                    if response_msg_type == "DISCOVERY_RESPONSE":
                        ...
                        discovery_logger.info(f"Final check before sending DISCOVERY_RESPONSE (CorrID: {correlation_id}): status='{status_in_payload}', results_count={results_count}")
                
                response_to_send = copy.deepcopy(response_message) 
                try:
                    response_json = safe_json_serialize(response_to_send)
                except (NameError, AttributeError):
                    discovery_logger.warning("safe_json_serialize not available, falling back to standard json.dumps")
                ...
                discovery_logger.info(f"Sent response (Type: {sent_msg_type}) to '{sender_id}' (CorrID: {correlation_id})")
                
                log_snippet = response_json[:250] + "..." if len(response_json) > 250 else response_json
                discovery_logger.debug(f"Response sent (CorrID: {correlation_id}): {log_snippet}")
            except Exception as e:
                discovery_logger.error(f"Failed to send response to '{sender_id}' (CorrID: {correlation_id}): {e}", exc_info=True)
        else:
            discovery_logger.warning(f"Cannot send response: Sender '{sender_id}' (CorrID: {correlation_id}) disconnected")

async def handle_registration(payload: Dict[str, Any], sender_id: str, correlation_id: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    if not agent_id_from_payload or not isinstance(agent_id_from_payload, str) or agent_id_from_payload != sender_id:
        discovery_logger.warning(f"Registration failed for sender '{sender_id}' (CorrID: {correlation_id}): Invalid/mismatched agent_id '{agent_id_from_payload}' in payload.")
        error_payload = {"error_code": "E002", "error_message": "Registration requires valid 'agent_id' in payload matching WebSocket sender_id"}
    ...
    if missing_fields:
        discovery_logger.warning(f"Registration failed for '{agent_id_from_payload}' (CorrID: {correlation_id}): Incomplete payload. Missing: {missing_fields}")
        error_payload = {"error_code": "E003", "error_message": f"Incomplete registration. Missing: {', '.join(missing_fields)}"}
        return create_mcp_error_response(sender_id, correlation_id, "MCP_DISCOVERY", error_payload)
    
    capabilities_list = payload.get("capabilities", [])
    if not isinstance(capabilities_list, list):
        discovery_logger.warning(f"Registration failed for '{agent_id_from_payload}' (CorrID: {correlation_id}): Invalid 'capabilities' format (expected list).")
    ...
    discovery_logger.info(f"Agent '{agent_id_from_payload}' (Type: {registration_info.get('agent_type')}) registered/updated. Status: '{registration_info.get('status')}'. Caps: {registered_caps}. Total Agents: {current_registry_size}. (CorrID: {correlation_id})")
...
async def handle_query(payload: Dict[str, Any], sender_id: str, correlation_id: Optional[str]) -> Dict[str, Any]:
    ...
    if not query_by_agent_id_val and not query_by_capability_list_val:
        discovery_logger.warning(f"Invalid query from '{sender_id}' (CorrID: {correlation_id}): No query parameters provided (query_by_agent_id or query_by_capability_list).")
        error_payload = {"error_code": "E101", "error_message": "Query must include either query_by_agent_id or query_by_capability_list"}
        return create_mcp_error_response(sender_id, correlation_id, "MCP_DISCOVERY", error_payload)
    ...
    try:
        ...
        discovery_logger.debug(f"Querying against registry snapshot of size {len(registry_snapshot)} (CorrID: {correlation_id})")

        if query_by_agent_id_val:
            if not isinstance(query_by_agent_id_val, str):
                raise ValueError("'query_by_agent_id' must be a string")
            discovery_logger.info(f"Query by ID: '{query_by_agent_id_val}' (CorrID: {correlation_id})")
            ...
            if agent_info:
                ...
                discovery_logger.debug(f"  Agent '{query_by_agent_id_val}': Connected={is_connected}, Status='{agent_status}' (Required: '{required_status_val}')")
                if (required_status_val == "any" or agent_status == required_status_val) and is_connected:
                    ...
                    clean_info = {key_item: value_item for key_item, value_item in agent_info.items() if not key_item.startswith('_')}
                    query_results.append(clean_info)
                    discovery_logger.info(f"  -> Matched agent '{query_by_agent_id_val}' by ID.")
                else:
                    discovery_logger.info(f"  Agent '{query_by_agent_id_val}' found but did not meet status/connection criteria.")
            else:
                discovery_logger.info(f"  Agent '{query_by_agent_id_val}' not found in registry snapshot.")
            ...
            for cap_str_item in query_by_capability_list_val: 
                if isinstance(cap_str_item, str):
                    ...
                    discovery_logger.warning(f"  Non-string capability in query_by_capability_list: {cap_str_item} (CorrID: {correlation_id}). Skipping.")

            if not required_caps_set: 
                 raise ValueError("Effective 'query_by_capability_list' is empty after processing non-string items.")

            discovery_logger.info(f"Query by capabilities: {required_caps_set} (CorrID: {correlation_id})")
            discovery_logger.info(f"Checking {len(registry_snapshot)} registered agents (CorrID: {correlation_id})...")
            
            for agent_id_key, agent_info_val in registry_snapshot.items():  # Fixed variable names in loop
                discovery_logger.debug(f"  Evaluating agent '{agent_id_key}' (CorrID: {correlation_id}):")
                ...
                discovery_logger.debug(f"    Status: '{agent_status_val}' (Required: '{required_status_val.upper()}')")
                
                if is_connected and (required_status_val == "any" or agent_status_val == required_status_val):
                    ...
                    if capabilities_match:
                        # Fixed variable names in dictionary comprehension
                        clean_info = {key_item: value_item for key_item, value_item in agent_info_val.items() if not key_item.startswith('_')}
                        query_results.append(clean_info)
                        discovery_logger.info(f"  -> Matched agent '{agent_id_key}' for capabilities {required_caps_set}.")
                else:
                    discovery_logger.debug(f"    Agent '{agent_id_key}' skipped (connection/status requirements not met).")
            discovery_logger.info(f"Capability query complete (CorrID: {correlation_id}). Found {len(query_results)} matching agents.")
            
        status_str = "success" if query_results else "not_found"
        discovery_logger.info(f"Query from '{sender_id}' (CorrID: {correlation_id}) processing complete. Final Status: '{status_str}'. Found {len(query_results)} agents.")
        ...
        if query_results:
            ...
            discovery_logger.debug(f"Results for CorrID '{correlation_id}' include agent IDs: {result_ids}")
        ...
        discovery_logger.warning(f"Invalid query from '{sender_id}' (CorrID: {correlation_id}): {ve}. Payload: {payload}")
        error_payload = {"error_code": "E003", "error_message": str(ve)}
        return create_mcp_error_response(sender_id, correlation_id, "MCP_DISCOVERY", error_payload)
        
    except Exception as e: 
        discovery_logger.error(f"Unexpected error during query processing from '{sender_id}' (CorrID: {correlation_id}): {e}", exc_info=True)
        error_payload = {"error_code": "E999", "error_message": f"Internal server error during query processing: {str(e)}"}
        return create_mcp_error_response(sender_id, correlation_id, "MCP_DISCOVERY", error_payload)