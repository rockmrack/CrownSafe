    ...
    if _redis_pool is None or force_new:
        ...
        for attempt in range(REDIS_RETRY_ATTEMPTS):
            try:
                ...
                logger.debug(f"Redis connection attempt {attempt+1} with params: {REDIS_HOST}:{REDIS_PORT} DB={REDIS_DB}")
                ...
                if attempt < REDIS_RETRY_ATTEMPTS - 1:
                    ...
                    _redis_pool = None
                    raise ConnectionError(f"Could not connect to Redis after {REDIS_RETRY_ATTEMPTS} attempts: {e}") from e
