    ...
    def generate_response(self, messages: List[Dict], **kwargs) -> LLMResponse:
        ...
        # If no specific patient but we have a score, use score-based logic
            ...
            if score_found > 0.75:
                ...
                template["clinical_rationale"] = f"High evidence score ({score_found:.0%}) strongly supports approval based on clinical criteria."
            elif score_found < 0.25:
                template["approval_likelihood_percent"] = int(score_found * 100)
                template["decision_prediction"] = "Deny"
                template["confidence_score"] = min(0.9, 1 - score_found)
                template["clinical_rationale"] = f"Low evidence score ({score_found:.0%}) indicates multiple unmet criteria. Step therapy or other requirements not satisfied."
            else:
                template["approval_likelihood_percent"] = int(score_found * 100)
                template["confidence_score"] = 0.5 + abs(score_found - 0.5) * 0.5
        
        # Log what we're returning
        logger.debug(f"MockLLMProvider returning: {template['decision_prediction']} with {template['approval_likelihood_percent']}% likelihood")
...
# Utility functions
def build_structured_prompt(sections: Dict[str, Any], 
                          separator: str = "\n" + "="*50 + "\n") -> str:
...
class CostTracker:
    ...
    def get_all_sessions_report(self) -> Dict[str, Any]:
        ...
        return {
            name: self.get_session_report(name)
