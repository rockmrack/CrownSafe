...
async def forward_message(
    ...
    logger.info(f"FORWARDING MSG [{label}]: Type='{msg_type}', OriginalSender='{hdr.get('sender_id', 'UNKNOWN')}', Target='{target_agent_id}', CorrID='{corr_id}'")
    
    message_content_snippet = "Could not serialize for logging"
    try:
        message_content_snippet = json.dumps(message_to_forward)
        if len(message_content_snippet) > 300: message_content_snippet = message_content_snippet[:300] + "..."
    ...
    if not ws:
        logger.warning(f"FORWARDING MSG [{label}]: No active WebSocket connection for target '{target_agent_id}'. Cannot deliver.")
        return False
    ...
    try:
        ...
        logger.error(f"FORWARDING MSG [{label}]: Error delivering Type='{msg_type}' to '{target_agent_id}': {e}", exc_info=True)
        return False
...
async def handle_message(agent_id: str, message_text: str, websocket: WebSocket):
    ...
    try:
        ...
        logger.warning(f"MCP_ROUTER_RAW_RECV: Could not parse JSON for detailed critical log from '{agent_id}'. Raw: {log_snippet_critical}")
    except Exception: # Catch any other error during this pre-logging parse
        logger.warning(f"MCP_ROUTER_RAW_RECV: Error during pre-logging parse from '{agent_id}'. Raw: {log_snippet_critical}")
    ...
    try:
        ...
        if header_sender_id != agent_id: # Path agent_id must match sender_id in header
            logger.error(f"ROUTER: Sender ID mismatch in message from connection '{agent_id}': Header sender_id is '{header_sender_id}'. Rejecting message.")
            # Optionally send an error back to the client
            err_resp = create_mcp_error_response(
                sender_id="MCP_ROUTER", correlation_id=corr, target_agent_id=agent_id,
                payload={"error_code": "E001", "error_message": f"Header sender_id '{header_sender_id}' does not match connection agent_id '{agent_id}'."}
            )
            if err_resp: await websocket.send_json(err_resp)
            return
        ...
        if mtype == "PROCESS_USER_REQUEST":
            ...
            logger.debug(f"ROUTER: Searching for CommanderAgent among: {all_connected_agents} for PROCESS_USER_REQUEST from '{agent_id}'")
            for connected_agent_id_str in all_connected_agents:
                if "commander_agent" in connected_agent_id_str.lower(): # More specific check
                    ...
                    logger.info(f"ROUTER: Found CommanderAgent '{target_commander_id}' to handle PROCESS_USER_REQUEST from '{agent_id}'")
                    break
            ...
            if target_commander_id:
                ...
                if not success:
                    err_resp = create_mcp_error_response(
                        sender_id="MCP_ROUTER", correlation_id=corr, target_agent_id=agent_id,
                        payload={"error_code": "E008", "error_message": f"Could not forward PROCESS_USER_REQUEST to '{target_commander_id}'"}
                    )
                    if err_resp: await websocket.send_json(err_resp)
            else:
                ...
                err_resp = create_mcp_error_response(
                    sender_id="MCP_ROUTER", correlation_id=corr, target_agent_id=agent_id,
                    payload={"error_code": "E007", "error_message": "No CommanderAgent available to process request"}
                )
                if err_resp: await websocket.send_json(err_resp)
            return
        ...
        if mtype == "TASK_ASSIGN": # This is for agent-to-agent task assignment
            logger.info(f"ROUTER HANDLE: TASK_ASSIGN from='{agent_id}' to='{target_agent_id_for_task}', CorrID='{corr}'")
            if not target_agent_id_for_task:
                err_resp = create_mcp_error_response(sender_id="MCP_ROUTER", correlation_id=corr, target_agent_id=agent_id, payload={"error_code": "E004", "error_message": "Missing target_agent_id for TASK_ASSIGN"})
                if err_resp: await websocket.send_json(err_resp)
                return
            # The sender_id for forward_message should be the original sender (agent_id from path)
            success = await forward_message(agent_id, msg, target_agent_id_for_task, label="TASK_ASSIGN")
            if not success:
                err_resp = create_mcp_error_response(sender_id="MCP_ROUTER", correlation_id=corr, target_agent_id=agent_id, payload={"error_code": "E005", "error_message": f"Could not forward TASK_ASSIGN to '{target_agent_id_for_task}'"})
                if err_resp: await websocket.send_json(err_resp)
            return

        if mtype in ("TASK_COMPLETE", "TASK_FAIL"): # TASK_ACK was removed from this group as it's usually server->client
            original_requester = target_agent_id_for_task 
            logger.info(f"ROUTER HANDLE: {mtype} from='{agent_id}' routing to original_requester='{original_requester}', CorrID='{corr}'")
            if not original_requester:
                logger.warning(f"ROUTER: No target_agent_id (original_requester_id) on {mtype} from '{agent_id}'; dropping.")
                return
            # The sender_id for forward_message should be the original sender (agent_id from path)
        ...
        if mtype == "PING":
            ...
            pong_payload = {"timestamp": payload.get("timestamp") if isinstance(payload, dict) else datetime.now(timezone.utc).timestamp(), "status": "acknowledged"}
            # PONG is sent from MCP_ROUTER back to the pinger (agent_id)
            pong_msg = create_mcp_message(sender_id="MCP_ROUTER", target_agent_id=agent_id, message_type="PONG", payload=pong_payload, correlation_id=corr)
            if pong_msg: await websocket.send_json(pong_msg)
            else: logger.error(f"ROUTER: Unable to build PONG for '{agent_id}', CorrID='{corr}'")
            return

        logger.warning(f"ROUTER HANDLE: Unhandled message_type='{mtype}', TargetService='{target_svc}', From='{agent_id}', CorrID='{corr}'")
        err_resp = create_mcp_error_response(sender_id="MCP_ROUTER", correlation_id=corr, target_agent_id=agent_id, payload={"error_code": "E003", "error_message": f"Unhandled message type '{mtype}' by router"})
        if err_resp: await websocket.send_json(err_resp)
        ...
        try:
            ...
            if err_resp_unparseable: await websocket.send_json(err_resp_unparseable)
        except Exception as send_err_ex: logger.error(f"ROUTER: Failed to send unparseable JSON error to '{agent_id}': {send_err_ex}")

    except Exception as e:
        logger.exception(f"ROUTER: Internal error handling message from '{agent_id}'")
        try:
            corr_id_for_error = msg.get("mcp_header", {}).get("correlation_id", "") if msg and isinstance(msg, dict) else None # Use None if not found
            err_resp = create_mcp_error_response(sender_id="MCP_ROUTER", correlation_id=corr_id_for_error, target_agent_id=agent_id, payload={"error_code": "E999", "error_message": "Internal router error processing message."})
            if err_resp: await websocket.send_json(err_resp)
        except Exception as send_err_ex: logger.error(f"ROUTER: Failed to send internal error notification to '{agent_id}': {send_err_ex}")