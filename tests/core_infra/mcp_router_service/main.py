...
if os.path.exists(env_path):
    ...
    bootstrap_logger.warning(f".env file not found at {env_path}. Service may rely on environment variables being set externally.")
...
try:
    ...
    bootstrap_logger.critical(f"CRITICAL IMPORT ERROR: Failed to import local modules: {e}. Check module structure, PYTHONPATH, or if running from the correct directory.", exc_info=True)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if hasattr(discovery, 'initialize_registry') and callable(discovery.initialize_registry):
            ...
            logger.warning("Lifespan: Agent registry not found or not a dict in discovery module. Cannot clear/initialize.")
        ...
        logger.critical(f"Lifespan: CRITICAL ERROR during startup initialization: {startup_e}", exc_info=True)
    ...
    try:
        if hasattr(state, 'close_all_connections') and callable(state.close_all_connections):
            ...
            logger.info("Lifespan: state.close_all_connections not found or not callable. Manual connection cleanup might be needed if connections persist.")
        
        logger.info("Lifespan: Resource cleanup during shutdown completed.")
    except Exception as shutdown_e:
         logger.error(f"Lifespan: Error during shutdown resource cleanup: {shutdown_e}", exc_info=True)
...
async def websocket_endpoint(websocket: WebSocket, agent_id: str):
    ...
    if not (hasattr(state, 'add_connection') and callable(state.add_connection)) or \
       not (hasattr(state, 'remove_connection') and callable(state.remove_connection)) or \
       not (hasattr(state, 'get_all_connections') and callable(state.get_all_connections)) or \
       not callable(handle_message):
        app_logger.critical("CRITICAL: state module or handle_message function is not properly initialized/imported. WebSocket endpoint cannot function.")
    ...
    if not connection_added:
        app_logger.warning(f"Failed to add connection for agent '{agent_id}', invalid agent_id or other error. Closing.")
        await websocket.close(code=1008, reason="Failed to register connection.")
        return
        
    app_logger.info(f"WebSocket for agent '{agent_id}' accepted and added. Total connections: {len(state.get_all_connections())}")
    ...
    try:
        ...
        app_logger.info(f"Agent '{agent_id}' disconnected (Code: {e.code}, Reason: '{e.reason if e.reason else 'N/A'}').")
    except Exception as e:
        app_logger.error(f"Unexpected error in WebSocket loop for agent '{agent_id}': {type(e).__name__} - {e}", exc_info=True)
        if websocket.client_state != websocket.client_state.DISCONNECTED:
            try:
                await websocket.close(code=1011, reason="Internal server error")
            except RuntimeError:
                app_logger.warning(f"RuntimeError attempting to close WebSocket for '{agent_id}', likely already closed.")
    finally:
        # FIXED: Properly handle return value
        removed = state.remove_connection(agent_id)
        if removed:
            app_logger.info(f"Connection for '{agent_id}' cleanly removed. Total connections: {len(state.get_all_connections())}")
        else:
            app_logger.info(f"Attempted to remove connection for '{agent_id}', but it was not found. Total connections: {len(state.get_all_connections())}")
...
if __name__ == "__main__":
    ...
    main_execution_logger.info(f"Uvicorn configured to run on host='{run_host}', port={run_port}, log_level='{run_log_level}', reload={run_reload}")
