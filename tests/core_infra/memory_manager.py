...
class MemoryManager:
    def __init__(self, 
        ...
        if logger_instance is not None:
            ...
            if hasattr(logger_instance, 'info') and hasattr(logger_instance, 'error') and hasattr(logger_instance, 'warning'):
                self.logger = logger_instance
            else:
                # If it's not a proper logger (e.g., string), create a new one
                print(f"WARNING: Invalid logger_instance provided (type: {type(logger_instance)}). Creating new logger.")
    ...
    def _merge_metadata_for_existing_document(self, 
                                            existing_metadata: Dict[str, Any], 
                                            new_context_metadata: Dict[str, Any], 
                                            current_timestamp: str) -> Dict[str, Any]:
        ...
        merged['user_goals_context'] = _aggregate_context_list('user_goals_context', new_context_metadata.get('user_goal_context'))
        merged['drug_names_context'] = _aggregate_context_list('drug_names_context', new_context_metadata.get('drug_name_context'))
        merged['disease_names_context'] = _aggregate_context_list('disease_names_context', new_context_metadata.get('disease_name_context'))

        # Update other fields with better data
        for key, value in new_context_metadata.items():
            if (key not in ['workflow_id_source', 'user_goal_context', 'drug_name_context', 
                           'disease_name_context', 'timestamp_added_or_updated'] 
        ...
        try:
            ...
            if existing_docs and existing_docs.get('ids'):
                for i, doc_id in enumerate(existing_docs['ids']):
                    if (doc_id and existing_docs.get('metadatas') and 
                        i < len(existing_docs['metadatas']) and 
                        existing_docs['metadatas'][i] is not None):
    ...
    def _prepare_safe_metadata(self, base_metadata: Dict[str, Any], workflow_id: str, 
                              user_goal: str, drug_name: Optional[str], 
                              disease_name: Optional[str], timestamp: str) -> Dict[str, Any]:
    ...
    def store_workflow_outputs(self, 
                               workflow_id: str, 
                               user_goal: str, 
                               extracted_entities: Dict[str, Optional[str]], 
                               pubmed_results_payload: Optional[Dict[str, Any]], 
                               clinical_trials_payload: Optional[Dict[str, Any]], 
                               drug_safety_payload: Optional[Dict[str, Any]], 
                               pdf_path: Optional[str] = None,
                               completion_timestamp: Optional[str] = None) -> Dict[str, Any]:
        ...
        if clinical_trials_payload and isinstance(clinical_trials_payload.get("trials_data"), list):
            for trial_item in clinical_trials_payload.get("trials_data", []):
                ...
                elif hasattr(trial_item, 'nct_id'):
                    trial_dict = asdict(trial_item)
                else:
                    continue
                
                if not trial_dict.get("nct_id"):
                    continue
                    
                nct_id = str(trial_dict["nct_id"])
                doc_id = self._generate_canonical_id("trial", nct_id)
                ids_for_upsert.append(doc_id)
                
                trial_text = (f"Clinical Trial: {trial_dict.get('title', 'N/A')}. "
                             f"Status: {trial_dict.get('status', 'N/A')}. "
                             f"Conditions: {trial_dict.get('condition', 'N/A')}. "
                             f"Interventions: {trial_dict.get('intervention', 'N/A')}.")
                docs_for_upsert.append(trial_text)
                ...
                trial_metadata = {
                    ...
                    "url": trial_dict.get("url"),
                    "source_agent_id": "clinical_trials_agent", 
                    **context_meta
                }
                metadatas_to_prepare.append(trial_metadata)
        ...
        if drug_safety_payload and drug_safety_payload.get("drug_queried"):
            ...
            safety_text = (f"Drug Safety Summary for {drug_queried}. "
                          f"Total Adverse Event Reports: {drug_safety_payload.get('total_reports', 0)}. "
                          f"Top Reported Reactions: {reactions_summary}.")
            docs_for_upsert.append(safety_text)
            ...
            metadatas_to_prepare.append(safety_metadata)
        ...
        for i, doc_id in enumerate(ids_for_upsert):
            if doc_id in existing_metadatas:
                ...
                new_metadata = self._prepare_safe_metadata(
                    metadatas_to_prepare[i], workflow_id, user_goal, drug_name, disease_name, current_utc_timestamp
                )
        ...
        try:
            ...
            self.logger.info(f"Upserting {len(docs_for_upsert)} documents. "
                           f"New: {num_new}, Updated: {num_updated}")
    ...
    def dump_collection_sample(self, 
                              document_type: Optional[str] = None, 
                              limit: int = 5,
                              parse_json: bool = True) -> None:
        """Print a sample of documents for debugging."""
        if not self.collection:
            print("Collection unavailable.")
            return
        
        print(f"\n--- Sample Dump: {self.collection_name} "
              f"(Limit: {limit}, Type: {document_type or 'Any'}) ---")
                ...
                if results.get('metadatas') and i < len(results['metadatas']):
                    ...
                    if metadata:
                        ...
                        for key, value in metadata.items():
                            if parse_json and key in ['referenced_in_workflows', 'user_goals_context', 
                                                    'drug_names_context', 'disease_names_context']:
