...
class SecurityLimitsTester:
    ...
    def test_cors_denied_origin(self) -> bool:
        ...
        for origin in evil_origins[:1]:  # Test first one
            ...
            if "access-control-allow-origin" in headers_lower:
                ...
                self.test(
                    allowed != origin,
                    f"Evil origin NOT echoed: {allowed} != {origin}"
                )
            else:
                self.test(
                    True,
                    "No Access-Control-Allow-Origin for evil origin"
                )
    ...
    def test_security_headers(self) -> bool:
        ...
        for header in optional_headers:
            if header.lower() in headers_lower:
                self.test(
                    True,
                    f"{header} present: {headers_lower[header.lower()][:50]}"
                )
    ...
    def test_ua_blocking(self) -> bool:
        ...
        self.test(
            ...
            f"Normal UA allowed: {response.status_code}"
        )
    ...
    def test_sql_injection_protection(self) -> bool:
        ...
        for payload in sql_payloads[:2]:
            ...
            if response.status_code == 200:
                ...
                self.test(
                    len(results) == 0,
                    "SQL injection returns no results"
                )
