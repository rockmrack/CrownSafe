...
class PrivacyEndpointTester:
    ...
    def test_legal_pages(self) -> bool:
        ...
        for path, expected_title in pages:
            ...
            self.test(
                ...
                f"{path} returns {response.status_code}"
            )
            ...
            if response.status_code == 200:
                ...
                self.test(
                    ...
                    f"{path} contains '{expected_title}'"
                )
                self.test(
                    "BabyShield" in content,
                    f"{path} contains BabyShield branding"
                )
            else:
                all_ok = False
    ...
    def test_privacy_summary(self) -> bool:
        ...
        response = self.session.get(
            f"{self.base_url}/api/v1/user/privacy/summary"
        )
        ...
        if response.status_code == 200:
            ...
            self.test(
                "dpo_email" in summary,
                f"DPO email: {summary.get('dpo_email')}"
            )
            self.test(
                "retention_periods" in summary,
    ...
    def test_admin_list_requests(self) -> bool:
        ...
        response = self.session.get(
            f"{self.base_url}/api/v1/admin/privacy/requests"
        )
        self.test(
        ...
        self.test(
            response.status_code == 200,
            f"Admin list returns {response.status_code}"
        )
        ...
        if response.status_code == 200:
            ...
            if items:
                ...
                self.test(
                    "kind" in first,
                    f"Request has kind: {first.get('kind')}"
                )
                self.test(
                    "status" in first,
                    f"Request has status: {first.get('status')}"
                )
                self.test(
    ...
    def test_admin_update_status(self) -> bool:
        ...
        self.test(
            response.status_code == 200,
            f"Valid update returns {response.status_code}"
        )
    ...
    def test_request_status_check(self) -> bool:
        ...
        if self.export_request_id:
            ...
            if response.status_code == 200:
                ...
                self.test(
                    "submitted_at" in status,
                    "Submission time provided"
                )
                self.test(
                    "sla_days" in status,
                    f"SLA days: {status.get('sla_days')}"
                )
                self.test(
    ...
    def test_privacy_headers(self) -> bool:
        ...
        self.test(
            "traceId" in data or "trace_id" in data,
    ...
    def test_admin_statistics(self) -> bool:
        ...
        if response.status_code == 200:
            ...
            self.test(
                "overview" in stats,
                "Has overview statistics"
            )
            self.test(
                "by_kind" in stats,
                "Has breakdown by request type"
            )
            self.test(
                "processing_metrics" in stats,
                "Has processing metrics"
            )
