...
class TestMetricsWithPrometheus:
    ...
    def test_inc_req_with_prometheus(self, mock_chat_req) -> None:
    ...
    def test_obs_total_with_prometheus(self, mock_chat_lat) -> None:
        obs_total(150)
        mock_chat_lat.observe.assert_called_once_with(150.0)

    @patch("core.metrics.PROM", True)
    @patch("core.metrics.TOOL_LAT")
    def test_obs_tool_with_prometheus(self, mock_tool_lat) -> None:
        mock_labels = MagicMock()
    ...
    def test_obs_synth_with_prometheus(self, mock_syn_lat) -> None:
        obs_synth(200)
...
class TestMetricsWithoutPrometheus:
    ...
    def test_inc_req_without_prometheus(self) -> None:
        # Should not raise any exceptions
        inc_req("conversation", "pregnancy_risk", ok=True, circuit=False)
        inc_req("explain", "unknown", ok=False, circuit=True)

    @patch("core.metrics.PROM", False)
    def test_obs_total_without_prometheus(self) -> None:
        # Should not raise any exceptions
        obs_total(150)
        obs_total(0)
        obs_total(5000)
    ...
    @patch("core.metrics.PROM", False)
    def test_obs_tool_without_prometheus(self) -> None:
        # Should not raise any exceptions
        obs_tool("pregnancy_risk", 50)
        obs_tool("unclear_intent", 1000)

    @patch("core.metrics.PROM", False)
    def test_obs_synth_without_prometheus(self) -> None:
        # Should not raise any exceptions
        obs_synth(100)
        obs_synth(2000)
...
class TestMetricsEdgeCases:
    @patch("core.metrics.PROM", True)
    @patch("core.metrics.CHAT_REQ")
    def test_inc_req_various_combinations(self, mock_chat_req) -> None:
        mock_labels = MagicMock()
        mock_chat_req.labels.return_value = mock_labels
    ...
    @patch("core.metrics.PROM", True)
    @patch("core.metrics.CHAT_LAT")
    def test_obs_total_edge_values(self, mock_chat_lat) -> None:
        # Test edge cases
        obs_total(0)
        obs_total(1)
        obs_total(10000)
...
class TestNoOpBehavior:
    ...
    def test_noop_class_methods(self) -> None:
...
class TestNewMetrics:
    ...
    def test_inc_fallback_with_prometheus(self, mock_chat_fallback) -> None:
    ...
    def test_inc_blocked_with_prometheus(self, mock_chat_blocked) -> None:
    ...
    @patch("core.metrics.PROM", False)
    def test_inc_fallback_without_prometheus(self) -> None:
        from core.metrics import inc_fallback
    ...
    @patch("core.metrics.PROM", False)
    def test_inc_blocked_without_prometheus(self) -> None:
        from core.metrics import inc_blocked
