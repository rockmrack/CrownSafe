...
from sqlalchemy import create_engine, Column, String, Boolean, Date, DateTime, ForeignKey, BigInteger, SmallInteger, JSON
...
class TestChatMemoryModels:
    ...
    def test_user_profile_creation(self, db_session):
        ...
        # Retrieve and verify
        retrieved = db_session.query(UserProfile).filter(
    ...
    def test_user_profile_defaults(self, db_session):
        ...
        retrieved = db_session.query(UserProfile).filter(
            UserProfile.user_id == "test-user-456"
        ).first()
    ...
    def test_user_profile_json_fields(self, db_session):
        ...
        retrieved = db_session.query(UserProfile).filter(
            UserProfile.user_id == "test-user-789"
        ).first()
    ...
    def test_conversation_creation(self, db_session):
        ...
        retrieved = db_session.query(Conversation).filter(
    ...
    def test_conversation_message_creation(self, db_session):
        ...
        retrieved = db_session.query(ConversationMessage).filter(
    ...
    def test_conversation_relationship(self, db_session):
        ...
        # Test relationship
        retrieved_conv = db_session.query(Conversation).filter(
            Conversation.id == "conv-relationship"
        ).first()
        ...
        retrieved_msg = db_session.query(ConversationMessage).filter(
            ConversationMessage.id == message1.id
        ).first()
    ...
    def test_conversation_cascade_delete(self, db_session):
        ...
        message_count = db_session.query(ConversationMessage).filter(
            ConversationMessage.conversation_id == "conv-cascade"
        ).count()
    ...
    def test_user_profile_erase_request(self, db_session):
        ...
        retrieved = db_session.query(UserProfile).filter(
            UserProfile.user_id == "test-erase-user"
        ).first()
    ...
    def test_message_role_validation(self, db_session):
        ...
        messages = db_session.query(ConversationMessage).filter(
            ConversationMessage.conversation_id == "conv-roles"
        ).all()
    ...
    def test_message_intent_field(self, db_session):
        ...
        messages = db_session.query(ConversationMessage).filter(
            ConversationMessage.conversation_id == "conv-intent"
        ).all()
    ...
    def test_message_trace_id_field(self, db_session):
        ...
        messages = db_session.query(ConversationMessage).filter(
            ConversationMessage.conversation_id == "conv-trace"
        ).all()
    ...
    def test_user_profile_pregnancy_fields(self, db_session):
        ...
        retrieved = db_session.query(UserProfile).filter(
            UserProfile.user_id == "test-pregnancy"
        ).first()
    ...
    def test_user_profile_allergies_edge_cases(self, db_session):
        ...
        retrieved1 = db_session.query(UserProfile).filter(
            UserProfile.user_id == "user-empty-allergies"
        ).first()
        ...
        retrieved2 = db_session.query(UserProfile).filter(
            UserProfile.user_id == "user-none-allergies"
        ).first()
...
class TestDatabaseCompatibility:
    ...
    def test_json_type_selection(self):
        ...
        JsonType = get_json_type(db_url_postgres)
        assert JsonType.__name__ == "JSONB"
        
        # Test SQLite
        db_url_sqlite = "sqlite:///test.db"
        JsonType = get_json_type(db_url_sqlite)
        assert JsonType.__name__ == "JSON"
    ...
    def test_uuid_type_selection(self):
        ...
        # Test PostgreSQL
        db_url_postgres = "postgresql://user:pass@localhost/db"
        UuidType = get_uuid_type(db_url_postgres)
        assert hasattr(UuidType, 'as_uuid')
        
        # Test SQLite
        db_url_sqlite = "sqlite:///test.db"
        UuidType = get_uuid_type(db_url_sqlite)
