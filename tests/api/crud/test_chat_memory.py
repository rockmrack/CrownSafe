...
class TestConversation(TestBase):
    ...
    messages = relationship("TestConversationMessage", back_populates="conversation", cascade="all, delete-orphan")
...
def test_log_message(db_session):
    ...
    log_message(db_session, conv, role="user", content=user_content, intent=None, trace_id="trace-123")
    ...
    log_message(db_session, conv, role="assistant", content=assistant_content, intent="pregnancy_risk", trace_id="trace-123")
