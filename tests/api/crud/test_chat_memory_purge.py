...
from sqlalchemy import create_engine, Column, String, Boolean, Date, DateTime, ForeignKey, Integer, SmallInteger, Text
...
def test_purge_conversations_for_user_with_conversations(db_session):
    ...
    msg2 = ConversationMessageModel(conversation_id=conv1.id, role="assistant", content=json.dumps({"summary": "Hi there"}))
    msg3 = ConversationMessageModel(conversation_id=conv2.id, role="user", content=json.dumps({"text": "Question"}))
    msg4 = ConversationMessageModel(conversation_id=conv3.id, role="user", content=json.dumps({"text": "Other user"}))
