...
class AdminToolsTester:
    ...
    def test_admin_unauthorized(self) -> bool:
        ...
        if response.status_code in (401, 403):
            ...
            self.test(
                "error" in data and "code" in data["error"],
                "Error has standard format"
            )
    ...
    def test_admin_runs_list(self) -> bool:
        ...
        response = self.session.get(
            f"{self.base_url}/api/v1/admin/runs",
            headers=self.admin_headers
        )
        ...
        self.test(
            response.status_code == 200,
            f"Runs list returns {response.status_code}"
        )
        
        if response.status_code == 200:
            data = response.json()
            self.test(
                "items" in data.get("data", {}),
    ...
    def test_admin_ingest_validation(self) -> bool:
        ...
        self.test(
            response.status_code == 400,
            f"Invalid mode returns {response.status_code}"
        )
    ...
    def test_admin_run_details(self) -> bool:
        ...
        self.test(
            response.status_code == 400,
            f"Invalid UUID returns {response.status_code}"
        )
        
        # Test with non-existent UUID
        fake_id = str(uuid.uuid4())
        response = self.session.get(
            f"{self.base_url}/api/v1/admin/runs/{fake_id}",
            headers=self.admin_headers
        )
    ...
    def test_admin_dashboard_html(self) -> bool:
        ...
        if response.status_code == 200:
            ...
            self.test(
                "BabyShield Admin" in content,
                "Dashboard contains expected title"
            )
            self.test(
                "apiKey" in content,
                "Dashboard has API key input"
            )
            self.test(
                "Data Freshness" in content,
                "Dashboard has freshness section"
            )
