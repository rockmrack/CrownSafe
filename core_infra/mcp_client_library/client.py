...
from websockets.exceptions import ConnectionClosedOK, ConnectionClosedError, InvalidURI, WebSocketException, InvalidStatus as WebsocketsInvalidStatus # Explicit import
...
class MCPClient:
    def __init__(
        ...
        self.logger.critical(f"MCPClient INSTANCE LOADED (Post-Claude-Review-ClientPy-LogLevel). Agent: {self.agent_id}") # Updated version marker
        self.logger.info(f"MCPClient initialized for agent '{self.agent_name}' (ID: {self.agent_id}, Type: {self.agent_type}) connecting to {self.ws_url}")

    def is_websocket_open(self) -> bool:
        if self.websocket is None: return False
        try: return self.websocket.state == WebSocketStateEnum.OPEN
        except AttributeError: self.logger.error(f"AttributeError accessing websocket.state for {self.agent_id}."); return False 
        except Exception as e: self.logger.error(f"Unexpected error checking WebSocket state for {self.agent_id}: {e}", exc_info=True); return False
    ...
    @retry(
        stop=stop_after_attempt(DEFAULT_MAX_CONNECT_ATTEMPTS), # Uses default, not instance specific max_connect_attempts
        wait=wait_fixed(DEFAULT_RECONNECT_DELAY),          # Uses default, not instance specific reconnect_delay
        retry=retry_if_exception_type((MCPConnectionError, ConnectionRefusedError, WebSocketException, WebsocketsInvalidStatus)),
        before_sleep=before_sleep_log 
    )
    async def connect(self):
        if self.is_connected: self.logger.info(f"Already connected ({self.agent_id})."); return
        ...
        try:
            ...
            if self.is_websocket_open():
                self._is_connected = True; self._stop_requested.clear()
                ...
                self.logger.error(f"connect() for {self.agent_id} completed but WebSocket not OPEN. State: {ws_state_str}")
                if self.websocket: await self.websocket.close()
                self.websocket = None; self._is_connected = False
            ...
            self._is_connected = False; self.websocket = None
            self.logger.error(f"Connection attempt to {self.ws_url} for {self.agent_id} timed out after {connect_timeout}s.")
            raise MCPConnectionError(f"Connection attempt timed out for {self.agent_id}")
        except (InvalidURI, ConnectionRefusedError, WebSocketException, WebsocketsInvalidStatus) as e:
            self._is_connected = False; self.websocket = None 
            if isinstance(e, InvalidURI): self.logger.critical(f"Invalid URI for {self.agent_id}: {self.ws_url}"); raise MCPConnectionError(f"Invalid URI: {self.ws_url}") from e
            elif isinstance(e, ConnectionRefusedError): self.logger.error(f"Connection refused by server at {self.ws_url} for {self.agent_id}"); raise MCPConnectionError(f"Connection refused: {self.ws_url}") from e
            elif isinstance(e, WebsocketsInvalidStatus): self.logger.error(f"Server rejected WebSocket for {self.agent_id}: {e.status_code} {e.reason if hasattr(e, 'reason') else 'N/A'}"); raise MCPConnectionError(f"Server rejected WebSocket connection: {e.status_code}") from e
            elif isinstance(e, WebSocketException): self.logger.error(f"WebSocket connection failed for {self.agent_id}: {e}", exc_info=True); raise MCPConnectionError(f"WebSocket connection failed: {e}") from e
        except Exception as e: 
            self._is_connected = False; self.websocket = None
            self.logger.critical(f"Critical unexpected error during connect for {self.agent_id}: {e}", exc_info=True)
            raise MCPConnectionError(f"Critical unexpected connection error: {e}") from e

    async def disconnect(self):
        if not self._is_connected and self.websocket is None: self.logger.info(f"Already disconnected ({self.agent_id})."); return
        self.logger.info(f"Disconnecting ({self.agent_id})..."); self._stop_requested.set() 
        ...
        if self.websocket and self.is_websocket_open(): 
            ...
            except Exception as e: self.logger.error(f"Error closing WebSocket for {self.agent_id}: {e}", exc_info=True)
        self.websocket = None; self._is_connected = False
        self._receive_task = None; self._heartbeat_task = None
        self.logger.info(f"Disconnection process complete for {self.agent_id}.")

    async def send_message(
        self, payload: Dict[str, Any], message_type: str,
        target_agent_id: Optional[str] = None, target_service: Optional[str] = None,
        correlation_id: Optional[str] = None 
    ):
        if not self.is_connected: raise MCPConnectionError(f"Cannot send message ({self.agent_id}), not connected.")
        if not message_type: raise ValueError("message_type is required.")
        ...
        header_args = {"sender_id": self.agent_id, "message_type": message_type, "target_agent_id": target_agent_id, "target_service": target_service}
        if correlation_id is not None: header_args["correlation_id"] = correlation_id
        ...
        try:
            ...
            if message_type == "PING":
                ...
                self.logger.info(f"Sent message Type='{message.mcp_header.message_type}', Target='{target_agent_id or target_service}', CorrID='{message.mcp_header.correlation_id}' from {self.agent_id}")
            ...
            self.logger.error(f"Failed to send message Type='{message_type}' ({self.agent_id}): {e}", exc_info=True)
            raise MCPError(f"Failed to send message ({self.agent_id}): {e}") from e

    async def register_self(self):
        self.logger.info(f"Registering {self.agent_id} (Type: {self.agent_type}) with capabilities: {self.capabilities}")
        payload = {"agent_id": self.agent_id, "agent_name": self.agent_name, "agent_type": self.agent_type, "capabilities": self.capabilities, "status": "active" }
        try: await self.send_message(payload=payload, message_type="DISCOVERY_REGISTER", target_service="MCP_DISCOVERY")
        except Exception as e: self.logger.error(f"Failed to send registration message for {self.agent_id}: {e}", exc_info=True)

    async def query_discovery(self, capabilities_to_query: List[str]) -> Optional[str]:
        if not (isinstance(capabilities_to_query, list) and all(isinstance(c, str) for c in capabilities_to_query) and capabilities_to_query):
        ...
        payload = {"requester_agent_id": self.agent_id, "query_by_capability_list": capabilities_to_query}
        self.logger.info(f"Sending discovery query (CorrID: {correlation_id_for_query}) for {capabilities_to_query} from {self.agent_id}")
        try:
            await self.send_message(payload=payload, message_type="DISCOVERY_QUERY", target_service="MCP_DISCOVERY", correlation_id=correlation_id_for_query )
            return correlation_id_for_query 
        except Exception as e: 
            self.logger.error(f"Failed to send discovery query (CorrID: {correlation_id_for_query}) for {self.agent_id}: {e}", exc_info=True)
            return None
    ...
    async def _receive_loop(self):
        ...
        while not self._stop_requested.is_set() and self._is_connected:
            if not self.is_websocket_open():
                self.logger.warning(f"Receive loop ({self.agent_id}): WebSocket not open. Breaking loop."); self._is_connected = False; break
            try:
                ...
                message_data = None; validated_message: Optional[MCPMessage] = None
                try:
                    ...
                    self.logger.info(f"CLIENT_RECV_JSON_SUCCESS ({self.agent_id}): JSON loaded. Type='{msg_type_for_log}', CorrID='{corr_id_for_log}'")
                    
                    self.logger.debug(f"CLIENT_RECV_PYDANTIC_VALIDATE_ATTEMPT ({self.agent_id}): Attempting MCPMessage.model_validate...")
                    validated_message = MCPMessage.model_validate(message_data) 
                    self.logger.info(f"CLIENT_RECV_PYDANTIC_SUCCESS ({self.agent_id}): MCPMessage validated. Type='{validated_message.mcp_header.message_type}', CorrID='{validated_message.mcp_header.correlation_id}'")
                    
                    if self.message_handler:
                        self.logger.debug(f"CLIENT_RECV_INVOKE_HANDLER ({self.agent_id}): Invoking handler for Type='{validated_message.mcp_header.message_type}'...")
                        if asyncio.iscoroutinefunction(self.message_handler):
                            asyncio.create_task(self.message_handler(validated_message)) 
                        else: 
                            self.logger.warning(f"Message handler for {self.agent_id} is synchronous. Consider making it async.")
                            self.message_handler(validated_message) 
                    else: self.logger.error(f"No message handler configured for {self.agent_id} to handle Type='{validated_message.mcp_header.message_type if validated_message else 'N/A'}'.")
                except json.JSONDecodeError as json_err: self.logger.error(f"CLIENT_RECV_JSON_FAIL ({self.agent_id}): {json_err}. Raw: {str(message_text)[:300]}")
                except Exception as validation_or_handler_err: 
                    self.logger.error(f"CLIENT_RECV_VALIDATE_OR_HANDLE_FAIL ({self.agent_id}): {validation_or_handler_err}", exc_info=True)
                    data_snippet = str(message_data)[:500] + "..." if message_data and len(str(message_data)) > 500 else str(message_data)
                    self.logger.error(f"CLIENT_RECV_FAIL_DATA ({self.agent_id}): Problematic data: {data_snippet}")
            except asyncio.TimeoutError: self.logger.debug(f"Receive loop ({self.agent_id}): Timeout. Checking connection."); continue
            except (ConnectionClosedOK, ConnectionClosedError, asyncio.CancelledError, WebSocketException) as e:
                level = logging.INFO if isinstance(e, (ConnectionClosedOK, asyncio.CancelledError)) else logging.ERROR
                self.logger.log(level, f"Receive loop ({self.agent_id}): Connection closed/cancelled: {type(e).__name__} - {e}"); self._is_connected = False; break 
            except Exception as e: self.logger.error(f"Receive loop ({self.agent_id}): Unexpected outer error: {e}", exc_info=True); self._is_connected = False; break 
        self.logger.info(f"Receive loop ended for {self.agent_id}. Stop: {self._stop_requested.is_set()}, Connected: {self._is_connected}")
    ...
    async def _send_heartbeat(self):
        ...
        while not self._stop_requested.is_set():
            try: await asyncio.wait_for(self._stop_requested.wait(), timeout=self.heartbeat_interval)
            except asyncio.TimeoutError: pass 
            except asyncio.CancelledError: self.logger.info(f"Heartbeat task cancelled during sleep for {self.agent_id}."); break
            if self._stop_requested.is_set(): break
            if not self.is_connected: self.logger.warning(f"Heartbeat ({self.agent_id}): Not connected. Stopping heartbeat."); break
            try:
                await self.send_message(payload={"client_timestamp": datetime.now(timezone.utc).timestamp()}, message_type="PING", target_service="MCP_ROUTER")
                # self.logger.debug(f"Sent PING from {self.agent_id}.") # Already logged by send_message at DEBUG for PING
            except asyncio.CancelledError: self.logger.info(f"Heartbeat cancelled during PING send for {self.agent_id}."); break
            except (MCPConnectionError, MCPError) as e: self.logger.error(f"Heartbeat send error for {self.agent_id}: {e}. Connection lost?. Stopping heartbeat."); self._is_connected = False; break
            except Exception as e: self.logger.error(f"Heartbeat ({self.agent_id}): Unexpected PING send error: {e}", exc_info=True); break 
