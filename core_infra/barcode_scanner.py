...
class BarcodeScanner:
    ...
    def _scan_with_opencv(self, image: Image.Image) -> List[ScanResult]:
        ...
        try:
            ...
            if self.opencv_barcode_detector:
                try:
                    retval, decoded_info, decoded_type, points = self.opencv_barcode_detector.detectAndDecode(gray)
                    if retval:
    ...
    def _is_gs1_format(self, data: str) -> bool:
        ...
        return (data.startswith(chr(29)) or  # FNC1
                data.startswith("01") or      # Common AI
                re.match(r'^\(?\d{2,4}\)', data))  # AI in parentheses
