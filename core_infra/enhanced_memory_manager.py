...
class EnhancedMemoryManager(MemoryManager):
    ...
    def __init__(self, chroma_db_path: str = "chroma_db_data", collection_name: str = "cureviax_knowledge_base_v1", logger_instance: Optional[logging.Logger] = None):
        """Initialize Enhanced Memory Manager"""
        
        # Initialize parent class first
        super().__init__(logger_instance=logger_instance, chroma_db_path=chroma_db_path, collection_name=collection_name)
        ...
        if self.chroma_client and self.collection:
            ...
            self.logger.info("EnhancedMemoryManager V2.0 initialized with temporal analysis and contradiction detection")
        else:
    ...
    def _init_enhanced_collections(self):
        ...
        try:
            ...
            self.gaps_collection = self.chroma_client.get_or_create_collection(
                name=f"{self.collection_name}_gaps",
                metadata={"hnsw:space": "cosine"}
            )
    ...
    async def store_workflow_outputs_enhanced(self, workflow_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hasattr(workflow_data, 'get'):
            ...
            standard_result = self.store_workflow_outputs(
                ...
                clinical_trials_payload=research_data.get('clinical_trials'),
                drug_safety_payload=research_data.get('drug_safety'),
                pdf_path=pdf_path,
    ...
    async def _analyze_temporal_patterns(self, entities: Dict[str, List[str]], workflow_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Temporal analysis completed: {len(temporal_results['patterns_detected'])} patterns detected")
    ...
    async def _detect_temporal_pattern(self, entity: str, historical_docs: List[Dict[str, Any]], current_time: datetime) -> Optional[TemporalPattern]:
            ...
            for drug in entities.get("drugs", []):
                if drug in [p.entity for p in self.temporal_patterns.values()]:
                    ...
                    if pattern.confidence_score > 0.7:
                        insights.append(f"{drug}: {pattern.trend_direction} research trend (confidence: {pattern.confidence_score:.2f})")
    ...
    async def _detect_contradictions(self, entities: Dict[str, List[str]], workflow_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Contradiction detection completed: {len(contradiction_results['contradictions_found'])} contradictions found")
    ...
    async def _find_contradictory_evidence(self, entity: str) -> List[Contradiction]:
        ...
        try:
            ...
            contradictions.extend(await self._analyze_statement_contradictions(
                entity, "efficacy", efficacy_statements
            ))
            
            # Detect contradictions in safety
            contradictions.extend(await self._analyze_statement_contradictions(
                entity, "safety", safety_statements
            ))
    ...
    async def _analyze_statement_contradictions(self, entity: str, category: str, statements: List[Tuple[str, str]]) -> List[Contradiction]:
            ...
            for i, (statement1, source1) in enumerate(statements):
                for j, (statement2, source2) in enumerate(statements[i+1:], i+1):
                    ...
                    for positive_term, negative_terms in opposing_terms.items():
                        if (positive_term.lower() in statement1.lower() and 
                            any(neg_term.lower() in statement2.lower() for neg_term in negative_terms)):
                            contradiction_found = True
                            conflicting_claims = [statement1, statement2]
                            break
                        elif (positive_term.lower() in statement2.lower() and 
                              any(neg_term.lower() in statement1.lower() for neg_term in negative_terms)):
                            contradiction_found = True
        ...
        try:
            ...
            for entity, contradictions in entity_contradictions.items():
                if len(contradictions) > 2:
                    suggestions.append(f"Consider systematic review of {entity} literature due to multiple contradictions")
    ...
    async def _identify_research_gaps(self, entities: Dict[str, List[str]], workflow_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _analyze_research_gaps_for_entity(self, entity: str) -> List[ResearchGap]:
        ...
        try:
            ...
            for doc in documents:
                ...
                if any(term in doc_lower for term in ["population", "demographic", "subgroup", "elderly", "pediatric"]):
    ...
    async def _prioritize_research_areas(self) -> List[str]:
        ...
        try:
            ...
            sorted_gaps = sorted(self.research_gaps.values(), key=lambda x: x.priority_score, reverse=True)
    ...
    async def _generate_cross_workflow_insights(self, entities: Dict[str, List[str]], workflow_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Cross-workflow insight generation completed: {len(insight_results['insights_generated'])} insights generated")
            ...
            for drug in drugs:
                ...
                if results["documents"] and results["documents"][0]:
                    for doc in results["documents"][0]:
                        ...
                        doc_lower = doc.lower()
                        outcomes = [
                            "cardiovascular outcomes", "heart failure", "kidney function",
                            "mortality", "hospitalization", "blood glucose", "weight loss"
                        ]
    ...
    async def _find_common_indications(self, drugs: List[str]) -> List[str]:
        ...
        try:
            ...
            common_indications = [indication for indication, count in indication_patterns.items() if count >= len(drugs) * 0.5]
        ...
        try:
            ...
            enhanced_analytics = {
                ...
                "temporal_patterns": {
                    ...
                    "high_confidence_patterns": len([p for p in self.temporal_patterns.values() if p.confidence_score > 0.8])
                },
                "contradictions": {
                ...
                "research_gaps": {
                    ...
                    "gaps_by_type": {
                        ...
                        for gap_type in ["clinical_trials", "safety_data", "mechanism_studies", "population_studies", "long_term_studies"]
                    },
                ...
                "cross_workflow_insights": {
                    ...
                    "insights_by_type": {
                        insight_type: len([i for i in self.cross_workflow_insights.values() if i.insight_type == insight_type])
                        for insight_type in ["drug_class_pattern", "indication_synergy", "safety_correlation"]
                    },
                    "high_confidence_insights": len([i for i in self.cross_workflow_insights.values() if i.confidence_score > 0.8])
                }
            }
    ...
    async def get_enhanced_research_recommendations(self, entities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            existing_evidence_results = await self._find_existing_evidence_for_entity(primary_drug, primary_disease, drug_class)
            ...
            self.logger.info(f"Research strategy determined: {strategy_analysis['strategy']} (confidence: {strategy_analysis['confidence']:.2f})")
            ...
            all_suggestions = []
            for category in ["gap_addressing", "contradiction_resolution", "cross_workflow_opportunities"]:
    ...
    async def _find_existing_evidence_for_entity(self, primary_drug: str, primary_disease: Optional[str] = None, drug_class: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            search_strategies = [
                ...
                },
                {
                    ...
                    "query": f"{drug_class} {primary_disease}" if drug_class and primary_disease else f"{primary_drug} class",
                    "weight": 0.8
                },
                {
                ...
                },
                {
                    ...
                    "query": f"{primary_disease} treatment {drug_class}" if primary_disease and drug_class else f"{primary_drug} {primary_disease}" if primary_disease else f"{primary_drug} indication",
                    "weight": 0.6
                }
            ]
                ...
                try:
                    ...
                    if results["metadatas"] and results["metadatas"][0]:
                        ...
                        evidence_results["search_strategies"][strategy["name"]] = {
                            ...
                            "best_distance": min([m["distance"] for m in strategy_matches]) if strategy_matches else 1.0,
                            "avg_distance": sum([m["distance"] for m in strategy_matches]) / len(strategy_matches) if strategy_matches else 1.0
                        }
                        
                        self.logger.debug(f"{strategy['name']}: {len(strategy_matches)} matches, best distance: {evidence_results['search_strategies'][strategy['name']]['best_distance']:.3f}")
            ...
            if all_matches:
                ...
                self.logger.debug(f"Analysis complete: {len(all_matches)} total matches, {len(similar_drugs)} similar drugs found")
    ...
    def _determine_research_strategy(self, evidence_results: Dict[str, Any], primary_drug: str, drug_class: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def _generate_strategy_specific_recommendations(self, strategy: str, primary_drug: str, evidence_results: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            if strategy == "comprehensive":
                ...
                if similar_drugs:
                    ...
                    recommendations["cross_workflow_opportunities"] = [r for r in recommendations["cross_workflow_opportunities"] if r]  # Remove empty strings
                ...
                if similar_drugs:
                    ...
                    recommendations["cross_workflow_opportunities"] = [r for r in recommendations["cross_workflow_opportunities"] if r]
            
            # Add general recommendations based on evidence
            if total_docs > 0:
                recommendations["priority_research"].append(f"Synthesize existing {primary_drug} evidence for clinical decision-making")
            
            if len(similar_drugs) >= 2:
                recommendations["cross_workflow_opportunities"].append(f"Explore {primary_drug} differentiation within drug class")
            
            self.logger.debug(f"Generated {len(recommendations['priority_research'])} priority recommendations")
            self.logger.debug(f"Generated {len(recommendations['cross_workflow_opportunities'])} cross-workflow opportunities")
