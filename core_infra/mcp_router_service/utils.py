        ...
        if isinstance(message_dict, dict) and "mcp_header" in message_dict and "payload" in message_dict:
            if isinstance(message_dict["mcp_header"], dict) and isinstance(message_dict["payload"], dict):
                 ...
                 if not header.get("message_type"):
                     logger.error(f"parse_mcp_message: Missing required field 'message_type' in header. Msg: {message_text[:200]}")
                     return None
                 if not header.get("sender_id"): # Added check for sender_id
                     logger.error(f"parse_mcp_message: Missing required field 'sender_id' in header. Msg: {message_text[:200]}")
                     return None
                 return message_dict
            else:
                 logger.error(f"parse_mcp_message: Invalid structure - header/payload not dicts. Msg: {message_text[:200]}")
                 return None
        else:
            logger.error(f"parse_mcp_message: Parsed JSON lacks basic MCP structure (mcp_header/payload keys). Msg: {message_text[:200]}")
            return None
    except json.JSONDecodeError:
        logger.error(f"parse_mcp_message: Failed JSON decode for message: {message_text[:200]}", exc_info=True)
        return None
    except Exception as e:
        logger.error(f"parse_mcp_message: Unexpected error parsing message: {e}. Msg: {message_text[:200]}", exc_info=True)
        return None
...
def create_mcp_message(
    ...
    try:
        ...
        if not isinstance(payload, dict):
            logger.warning(f"Payload for message type '{message_type}' (CorrID: {current_correlation_id}) is not a dict (type: {type(payload)}). Wrapping in 'data' field.")
            message_payload_safe = copy.deepcopy({"data": payload})
        else:
            message_payload_safe = copy.deepcopy(payload)

        message = {
            "mcp_header": cleaned_header,
            "payload": message_payload_safe
        }
        logger.debug(f"Created MCP Message: Type='{message_type}', Sender='{sender_id}', Target='{target_agent_id or target_service}', CorrID='{current_correlation_id}'")
        return message
    except Exception as e:
        logger.error(f"Error creating MCP message (Type: {message_type}, CorrID: {correlation_id}): {e}", exc_info=True)
        return None
...
def create_mcp_response(
    ...
    if response_message_type == "DISCOVERY_RESPONSE" and isinstance(payload_copy, dict):
        ...
        if not isinstance(results, list): # Ensure results is a list
            logger.warning(f"DISCOVERY_RESPONSE payload 'results' (CorrID: {correlation_id}) is not a list (Type: {type(results)}). Correcting to empty list.")
            results = []
            payload_copy["results"] = results

        current_results_count = len(results)
        logger.info(f"Processing {response_message_type} for Target='{target_agent_id}', CorrID='{correlation_id}': Initial status='{status}', initial results_count={current_results_count}")
            
        corrected_status = status
        if status == "success" and not results:
            logger.warning(f"Fixing inconsistent DISCOVERY_RESPONSE (CorrID: {correlation_id}): 'success' status with empty results. Changing status to 'not_found'.")
            corrected_status = "not_found"
        elif status == "not_found" and results:
            logger.warning(f"Fixing inconsistent DISCOVERY_RESPONSE (CorrID: {correlation_id}): 'not_found' status with {current_results_count} results. Changing status to 'success'.")
            corrected_status = "success"
        elif not status: # If status is empty or None
            corrected_status = "success" if results else "not_found"
            logger.warning(f"Fixing DISCOVERY_RESPONSE (CorrID: {correlation_id}): Missing status. Based on results ({current_results_count}), setting status to '{corrected_status}'.")
        
        if corrected_status != status: # Only update if changed
            payload_copy["status"] = corrected_status
            logger.info(f"DISCOVERY_RESPONSE (CorrID: {correlation_id}): Status corrected from '{status}' to '{corrected_status}'.")
    ...
    payload_log_snippet = {k: v for k, v in payload_copy.items() if k != "results"} if isinstance(payload_copy, dict) else str(payload_copy)
    results_count_for_log = len(payload_copy.get("results", [])) if isinstance(payload_copy, dict) and "results" in payload_copy else "N/A"
    
    logger.debug(f"Final response payload for create_mcp_message (CorrID: {correlation_id}): {payload_log_snippet}, results_count={results_count_for_log}, status='{payload_copy.get('status', 'N/A') if isinstance(payload_copy, dict) else 'N/A'}'")
    
    if isinstance(payload_copy, dict) and payload_copy.get("results") and isinstance(payload_copy["results"], list) and payload_copy["results"]:
        first_result_snippet = str(payload_copy["results"][0])[:100] + "..." if len(str(payload_copy["results"][0])) > 100 else str(payload_copy["results"][0])
...
def create_mcp_error_response(
    ...
    if "error_code" not in error_payload or "error_message" not in error_payload:
        logger.warning(f"Creating error response ({error_message_type}) for CorrID '{correlation_id}' without standard error_code/error_message. Adding defaults.")
        error_payload.setdefault("error_code", "E_UNKNOWN")
        error_payload.setdefault("error_message", "An unspecified error occurred.")

    logger.debug(f"Creating error response: Type='{error_message_type}', Sender='{sender_service}', Target='{target_agent_id}', CorrID='{correlation_id}'")
