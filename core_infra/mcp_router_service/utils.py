...
def parse_mcp_message(message_text: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if (
            ...
            logger.error(
                f"parse_mcp_message: Parsed JSON lacks basic MCP structure (mcp_header/payload keys). Msg: {message_text[:200]}"
            )
            return None
...
def create_mcp_message(
    ...
    try:
        ...
        if not isinstance(payload, dict):
            logger.warning(
                f"Payload for message type '{message_type}' (CorrID: {current_correlation_id}) is not a dict (type: {type(payload)}). Wrapping in 'data' field."
            )
        ...
        logger.debug(
            f"Created MCP Message: Type='{message_type}', Sender='{sender_id}', Target='{target_agent_id or target_service}', CorrID='{current_correlation_id}'"
        )
        return message
    except Exception as e:
        logger.error(
...
def create_mcp_response(
    ...
    if response_message_type == "DISCOVERY_RESPONSE" and isinstance(payload_copy, dict):
        ...
        if not isinstance(results, list):  # Ensure results is a list
            logger.warning(
                f"DISCOVERY_RESPONSE payload 'results' (CorrID: {correlation_id}) is not a list (Type: {type(results)}). Correcting to empty list."
            )
            results = []
        ...
        logger.info(
            f"Processing {response_message_type} for Target='{target_agent_id}', CorrID='{correlation_id}': Initial status='{status}', initial results_count={current_results_count}"
        )

        corrected_status = status
        if status == "success" and not results:
            logger.warning(
                f"Fixing inconsistent DISCOVERY_RESPONSE (CorrID: {correlation_id}): 'success' status with empty results. Changing status to 'not_found'."
            )
            corrected_status = "not_found"
        elif status == "not_found" and results:
            logger.warning(
                f"Fixing inconsistent DISCOVERY_RESPONSE (CorrID: {correlation_id}): 'not_found' status with {current_results_count} results. Changing status to 'success'."
            )
            corrected_status = "success"
        elif not status:  # If status is empty or None
            corrected_status = "success" if results else "not_found"
            logger.warning(
                f"Fixing DISCOVERY_RESPONSE (CorrID: {correlation_id}): Missing status. Based on results ({current_results_count}), setting status to '{corrected_status}'."
            )

        if corrected_status != status:  # Only update if changed
            payload_copy["status"] = corrected_status
            logger.info(
                f"DISCOVERY_RESPONSE (CorrID: {correlation_id}): Status corrected from '{status}' to '{corrected_status}'."
            )
    ...
    logger.debug(
        f"Final response payload for create_mcp_message (CorrID: {correlation_id}): {payload_log_snippet}, results_count={results_count_for_log}, status='{payload_copy.get('status', 'N/A') if isinstance(payload_copy, dict) else 'N/A'}'"
    )
...
def create_mcp_error_response(
    ...
    if "error_code" not in error_payload or "error_message" not in error_payload:
        logger.warning(
            f"Creating error response ({error_message_type}) for CorrID '{correlation_id}' without standard error_code/error_message. Adding defaults."
        )
