...
class BarcodeValidator:
    ...
    def _is_qr_code(self, barcode: str) -> bool:
        ...
        return (bool(re.match(url_pattern, barcode)) or
                bool(re.match(json_pattern, barcode)) or
                bool(re.match(base64_pattern, barcode)) or
                len(barcode) > 50)  # QR codes are typically longer
    ...
    def _is_data_matrix(self, barcode: str) -> bool:
        ...
        return (len(barcode) > 20 and 
                not barcode.isalnum() and
                any(ord(c) > 127 for c in barcode))
