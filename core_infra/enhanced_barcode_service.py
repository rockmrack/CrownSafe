...
class EnhancedBarcodeService:
    ...
    async def scan_barcode_exact(self, barcode: str, user_id: int) -> ExactScanResult:
        ...
        try:
            ...
            exact_matches = await self._find_exact_product_matches(
                validation_result.normalized_barcode,
                validation_result.barcode_type
            )
    ...
    async def _find_exact_product_matches(self, normalized_barcode: str, barcode_type: BarcodeType) -> List[Dict[str, Any]]:
    ...
    def _build_search_conditions(self, normalized_barcode: str, barcode_type: BarcodeType):
        ...
        if barcode_type in [BarcodeType.UPC_A, BarcodeType.UPC_E, BarcodeType.EAN_13, BarcodeType.EAN_8]:
            conditions.extend([
    ...
    def _calculate_match_confidence(self, normalized_barcode: str, product: RecallDB, barcode_type: BarcodeType) -> float:
    ...
    def _calculate_overall_confidence(self, validation_result: BarcodeValidationResult, matches: List[Dict[str, Any]]) -> float:
    ...
    def _get_scan_recommendations(self, validation_result: BarcodeValidationResult, matches: List[Dict[str, Any]]) -> List[str]:
