...
from core_infra.risk_assessment_models import (
    ...
    CompanyComplianceProfile, DataIngestionJob, ProductDataSource,
    DataSource
)
...
def sync_eu_safety_gate(days_back: int = 30):
    ...
    try:
        ...
        asyncio.set_event_loop(loop)
        records = loop.run_until_complete(
            connector.fetch_alerts(start_date=start_date)
        )
...
def recalculate_affected_products(days_back: int = 7):
    ...
    try:
        ...
        recent_products = db.query(ProductGoldenRecord).join(
            SafetyIncident
        ).filter(
            SafetyIncident.created_at >= cutoff_date
        ).distinct().all()
...
def recalculate_high_risk_scores():
    ...
    try:
        ...
        high_risk_profiles = db.query(ProductRiskProfile).filter(
...
def update_company_compliance():
    ...
    try:
        ...
        for (manufacturer,) in manufacturers:
            ...
            profile = db.query(CompanyComplianceProfile).filter(
                CompanyComplianceProfile.company_name == manufacturer
            ).first()
            ...
            recall_count = db.query(ProductGoldenRecord).filter(
                ProductGoldenRecord.manufacturer == manufacturer
            ).join(SafetyIncident).distinct().count()
            
            profile.total_recalls = recall_count
            
            # Recent recalls (last 12 months)
            cutoff = datetime.utcnow() - timedelta(days=365)
            recent_recalls = db.query(ProductGoldenRecord).filter(
                ProductGoldenRecord.manufacturer == manufacturer
            ).join(SafetyIncident).filter(
                SafetyIncident.created_at >= cutoff
            ).distinct().count()
    ...
    try:
        ...
        if scan_result.codes:
            ...
            if code_data.get("expiry_date"):
                product.manufacturing_date_range = {
...
def _find_or_create_product_from_record(record: SafetyDataRecord, db: Session) -> Optional[ProductGoldenRecord]:
    ...
    if not product and record.product_name:
        ...
        product = db.query(ProductGoldenRecord).filter(
            ProductGoldenRecord.product_name.ilike(f"%{record.product_name}%")
        ).first()
...
def _create_incident_from_record(record: SafetyDataRecord, product_id: str, db: Session) -> Optional[SafetyIncident]:
    ...
    existing = db.query(SafetyIncident).filter_by(
...
def _update_product_data_source(product_id: str, record: SafetyDataRecord, db: Session):
    ...
    existing = db.query(ProductDataSource).filter_by(
        product_id=product_id,
