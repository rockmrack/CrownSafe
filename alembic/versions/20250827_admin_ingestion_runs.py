...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), 
                  server_default=sa.text("CURRENT_TIMESTAMP")),  # Added
        sa.Column('metadata_json', postgresql.JSONB, nullable=True)  # Added for extra data
    )
    
    # Create indexes for efficient querying
    op.create_index('ix_ingestion_runs_agency_started', 
                     'ingestion_runs', 
                     ['agency', 'started_at'], 
                     unique=False)
    
    op.create_index('ix_ingestion_runs_status', 
                     'ingestion_runs', 
                     ['status'], 
                     unique=False)
    
    op.create_index('ix_ingestion_runs_created_at', 
                     'ingestion_runs', 
                     ['created_at'], 
                     unique=False)
