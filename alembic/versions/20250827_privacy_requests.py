...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('status', sa.String(16), nullable=False, 
                  server_default='queued'),  # queued|verifying|processing|done|rejected|expired
    ...
    op.create_index('ix_privacy_email_hash', 
                     'privacy_requests', 
                     ['email_hash'], 
                     unique=False)
    
    op.create_index('ix_privacy_status', 
                     'privacy_requests', 
                     ['status'], 
                     unique=False)
    
    op.create_index('ix_privacy_submitted_at', 
                     'privacy_requests', 
                     ['submitted_at'], 
                     unique=False)
    
    op.create_index('ix_privacy_kind_status', 
                     'privacy_requests', 
                     ['kind', 'status'], 
                     unique=False)
