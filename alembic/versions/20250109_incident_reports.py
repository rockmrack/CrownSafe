...
def upgrade():
    ...
    op.create_table('incident_reports',
        ...
        # Incident details
        sa.Column('incident_type', sa.Enum('INJURY', 'CHOKING_HAZARD', 'SHARP_EDGES', 'TOXIC_MATERIAL', 
                                          'STRUCTURAL_FAILURE', 'BURN_HAZARD', 'STRANGULATION', 
                                          'SUFFOCATION', 'ALLERGIC_REACTION', 'OTHER', 
                                          name='incidenttype'), nullable=False),
        ...
        # Severity
        sa.Column('severity_level', sa.Enum('MINOR', 'MODERATE', 'SEVERE', 'CRITICAL', 
                                           name='severitylevel'), nullable=False),
        ...
        # Status
        sa.Column('status', sa.Enum('SUBMITTED', 'UNDER_REVIEW', 'VERIFIED', 'FORWARDED_TO_AGENCY', 
                                   'RESOLVED', 'DUPLICATE', name='incidentstatus'), 
                 nullable=True),
    ...
    op.create_index(op.f('ix_incident_reports_barcode'), 'incident_reports', ['barcode'], unique=False)
    op.create_index(op.f('ix_incident_reports_brand_name'), 'incident_reports', ['brand_name'], unique=False)
    op.create_index(op.f('ix_incident_reports_cluster_id'), 'incident_reports', ['cluster_id'], unique=False)
    ...
    op.create_index(op.f('ix_incident_reports_product_name'), 'incident_reports', ['product_name'], unique=False)
    op.create_index(op.f('ix_incident_reports_severity_level'), 'incident_reports', ['severity_level'], unique=False)
    ...
    op.create_index(op.f('ix_incident_clusters_product_name'), 'incident_clusters', ['product_name'], unique=False)
    
    # Create agency_notifications table
    op.create_table('agency_notifications',
        sa.Column('id', sa.Integer(), nullable=False),
