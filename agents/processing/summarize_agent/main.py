...
if os.path.exists(dotenv_main_path): load_dotenv(dotenv_main_path)
...
async def handle_incoming_message(message: MCPMessage):
    ...
    try:
        ...
        logic_logger.info(f"MAIN_HANDLER: Received Type='{header.message_type}', From='{header.sender_id}', CorrID='{header.correlation_id}'.")
        ...
        response_from_logic = await agent_logic_instance.process_message(
            message_data=message_as_dict_for_logic, 
        ...
        if response_from_logic and isinstance(response_from_logic, dict):
            ...
            logic_logger.info(f"MAIN_HANDLER: Sending {response_message_type} for CorrID='{correlation_for_reply}' back to '{target_for_reply}'.")
            ...
            logic_logger.info(f"MAIN_HANDLER: Sent {response_message_type} response for CorrID='{correlation_for_reply}' to '{target_for_reply}'.")
        
        elif response_from_logic is not None:
             logic_logger.warning(f"Logic processed {header.message_type} but returned non-dict/non-None: {type(response_from_logic)}")

    except Exception as e:
        current_message_type_str = message.mcp_header.message_type if message and message.mcp_header else "UnknownType"
        logic_logger.error(f"MAIN_HANDLER: Error processing message (Type: {current_message_type_str}): {e}", exc_info=True)
        ...
        if current_message_type_str == "TASK_ASSIGN" and mcp_client_instance:
            try:
                ...
                logger_for_main.info(f"MAIN_HANDLER: Sending TASK_FAIL for CorrID {message.mcp_header.correlation_id} to {message.mcp_header.sender_id}")
                await mcp_client_instance.send_message(
                    payload=fail_payload, message_type="TASK_FAIL", 
...
async def main_agent_loop():
    ...
    logger_for_main.info(f"Initializing {AGENT_TYPE} '{AGENT_NAME}' (ID: {AGENT_ID}, Version: {AGENT_VERSION}) - Step 108")
    
    try:
        agent_logic_instance = SummarizeAgentLogic(agent_id=AGENT_ID, version=AGENT_VERSION, logger=logger_for_logic_passed)
    except Exception as e:
        logger_for_main.critical(f"Failed to initialize SummarizeAgentLogic: {e}.", exc_info=True); return

    mcp_settings = MCPConfig() 
    base_mcp_server_url = mcp_settings.DEFAULT_ROUTER_URL
    if "/ws/" in base_mcp_server_url: base_mcp_server_url = base_mcp_server_url.split('/ws/')[0]
    ...
    def signal_handler_func_wrapper(sig_name):
        if not stop_event.is_set(): logger_for_main.info(f"Shutdown signal {sig_name} received."); stop_event.set()
    for sig_name_enum in (signal.SIGINT, signal.SIGTERM):
        try: loop.add_signal_handler(sig_name_enum, lambda s=sig_name_enum.name: signal_handler_func_wrapper(s))
        except Exception as e: logger_for_main.warning(f"Cannot add signal handler for {sig_name_enum.name}: {e}.")
    ...
    try:
        ...
        else: logger_for_main.critical("MCPClient connection failed."); return
    except Exception as e: logger_for_main.critical(f"Unexpected error in main loop: {e}", exc_info=True)
    finally:
        logger_for_main.info(f"Shutting down {AGENT_ID}...");
        if mcp_client_instance and mcp_client_instance.is_connected: await mcp_client_instance.disconnect()
        for sig_name_enum in active_signal_handlers:
            try: loop.remove_signal_handler(sig_name_enum)
            except Exception: pass # Ignore errors removing handlers
