...
class SummarizeAgentLogic:
    def __init__(self, agent_id: str, version: str, logger: logging.Logger):
        ...
        if not openai_api_key:
            ...
            try:
                ...
                self.logger.error(f"Error initializing Langchain/OpenAI components for SummarizeAgent: {e}", exc_info=True)
                self.llm = None
                self.summarize_chain = None
    ...
    async def _generate_summary_with_llm(self, text_content: str) -> Optional[str]:
        ...
        try:
            ...
            text_splitter = RecursiveCharacterTextSplitter(chunk_size=10000, chunk_overlap=200) # Adjust chunk_size as needed
    ...
    async def process_message(self, message_data: Dict[str, Any], client: Any) -> Optional[Dict[str, Any]]:
        ...
        if message_type == "TASK_ASSIGN":
            ...
            if not input_data_for_summary:
                ...
                return {"message_type": "TASK_FAIL", "payload": {"workflow_id": payload.get("workflow_id"), "task_id": payload.get("task_id"), "agent_id": self.agent_id, "correlation_id": incoming_task_corr_id, "error_message": "No text provided for summarization."}}
            ...
            if isinstance(input_data_for_summary, dict) and "articles" in input_data_for_summary:
                ...
                return {"message_type": "TASK_FAIL", "payload": {"workflow_id": payload.get("workflow_id"), "task_id": payload.get("task_id"), "agent_id": self.agent_id, "correlation_id": incoming_task_corr_id, "error_message": "Invalid format for text_to_summarize."}}
            ...
            if "Error:" in summary_text: # Check if LLM call failed
                 return {"message_type": "TASK_FAIL", "payload": {"workflow_id": payload.get("workflow_id"), "task_id": payload.get("task_id"), "agent_id": self.agent_id, "correlation_id": incoming_task_corr_id, "error_message": summary_text}}
