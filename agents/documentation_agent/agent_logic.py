...
class DocumentationAgentLogic:
    ...
    def _register_custom_filters(self):
        ...
        self.jinja_env.filters['date_format'] = lambda x: datetime.strptime(x, '%Y-%m-%d').strftime('%B %d, %Y') if x else ''
        self.jinja_env.filters['escape_html'] = lambda x: str(x).replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;') if x else ''
    ...
    def _normalize_input_data(self, task_payload: dict) -> dict:
        ...
        additional_fields = ["patient_history", "drug_name", "diagnosis_codes", "patient_id", "insurer_id"]
        ...
        self.logger.debug(f"Decision: {report_data.get('decision_prediction')}, Confidence: {report_data.get('confidence_score')}")
    ...
    def _build_pa_summary_pdf(
        ...
        try:
            ...
            context = {
                ...
                "diagnosis_codes_formatted": ', '.join([self._safe_unicode_str(code) for code in original_request.get('diagnosis_codes', [])]),
        ...
        try:
            ...
            if original_request.get('allergies') or original_request.get('contraindications'):
                ...
                if original_request.get('allergies'):
                    md_parts.append(f"**Known Allergies:** {', '.join([self._safe_unicode_str(a) for a in original_request['allergies']])}")
    ...
    def _build_html_preview(
        ...
        try:
            ...
            return DocumentGenerationResult(
                success=False,
                error_message=f"HTML preview generation failed: {str(e)}"
            )
