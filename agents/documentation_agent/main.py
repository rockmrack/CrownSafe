...
class DocumentationAgentManager:
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    asyncio.get_event_loop().run_in_executor(
                        None,
                        self.documentation_logic.process_task,
                        task_payload
                    ),
        ...
        for attempt in range(1, Config.MAX_RETRIES + 1):
            try:
                ...
                if attempt < Config.MAX_RETRIES:
                    ...
                    logger.critical(f"[CRITICAL] All {Config.MAX_RETRIES} connection attempts failed for {self.agent_id}")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if not self.mcp_client or not self.mcp_client.is_connected:
                    logger.warning(f"[ALERT] Connection health check failed for {self.agent_id} - attempting reconnection")
                    ...
                    if hasattr(self.mcp_client, 'send_heartbeat'):
                        ...
                        logger.debug(f"Heartbeat sent - Status: {self.status.value}, Active tasks: {len(self.active_tasks)}")
    ...
    async def initialize_components(self):
        ...
        try:
            ...
            self.documentation_logic = DocumentationAgentLogic(
                agent_id=self.agent_id
            )
            ...
            logger.critical(f"Failed to initialize DocumentationAgent components: {e}", exc_info=True)
        ...
        try:
            ...
            if Config.ENABLE_HEALTH_CHECK:
                background_tasks.append(
                    asyncio.create_task(self._health_check_server(), name="health_check")
                )
            
            # Start metrics export if enabled
            if Config.ENABLE_METRICS:
                background_tasks.append(
                    asyncio.create_task(self._metrics_export_loop(), name="metrics_export")
                )
