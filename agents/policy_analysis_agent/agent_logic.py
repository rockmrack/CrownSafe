...
class PolicyAnalysisAgentLogic:
    ...
    def _normalize_task_name(self, task_name: str) -> str:
        ...
        task_mappings = {
            ...
            'check_coverage_criteria': ['coverage_check', 'criteria_check', 'check_criteria', 'evaluate_pa_criteria'],
    ...
    def get_policy_for_drug(self, drug_name: str) -> Dict[str, Any]:
        ...
        return {"status": "not_found", "message": f"Drug '{drug_name}' is not mentioned in this policy."}
    ...
    def check_coverage_criteria(self, drug_name: str, patient_evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if policy.get("status") != "Covered with Prior Authorization":
            return {"status": "success", "meets_criteria": False, "reason": f"Drug status is '{policy.get('status')}'."}

        criteria_list = policy.get("criteria", [])
        if not criteria_list:
            return {"status": "success", "meets_criteria": True, "reason": "No specific criteria listed."}
    ...
    def _evaluate_coverage_criteria(self, drug_name: str, policy_info: Dict[str, Any], 
                                  patient_data: Dict[str, Any]) -> CoverageDecision:
        ...
        if 'quantity_limits' in policy_info:
            ...
            if is_met:
                ...
                recommendations.append(f"Reduce quantity to {quantity_criterion['max_quantity']} units or less per fill")
        ...
        if not criteria_met:
            recommendations.insert(0, f"Prior authorization denied: {len(required_unmet)} required criteria not met")
        else:
    ...
    def _check_diagnosis_criterion(self, criterion: Dict[str, Any], 
                                 patient_data: Dict[str, Any]) -> tuple[bool, Dict[str, Any]]:
        ...
        details = {
            ...
            'message': f"Patient has {len(matching_codes)} of required diagnoses" if is_met
                      else "Patient missing required diagnosis"
        }
        
        return is_met, details

    def _check_step_therapy_criterion(self, criterion: Dict[str, Any], 
                                    patient_data: Dict[str, Any]) -> tuple[bool, Dict[str, Any]]:
        ...
        details = {
            ...
            'has_tried': has_tried,
            'message': f"Patient has tried {required_drug}" if has_tried
                      else f"Patient has not tried required medication: {required_drug}"
        }
        
        return has_tried, details

    def _check_lab_value_criterion(self, criterion: Dict[str, Any], 
                                 patient_data: Dict[str, Any]) -> tuple[bool, Dict[str, Any]]:
    ...
    def _check_age_limit_criterion(self, criterion: Dict[str, Any], 
                                 patient_data: Dict[str, Any]) -> tuple[bool, Dict[str, Any]]:
    ...
    def _check_quantity_limit_criterion(self, criterion: Dict[str, Any], 
                                      patient_data: Dict[str, Any]) -> tuple[bool, Dict[str, Any]]:
        ...
        return is_met, {
            'requested': requested_quantity,
            'maximum': max_quantity,
            'message': f"Quantity within limits" if is_met 
                      else f"Requested quantity {requested_quantity} exceeds maximum {max_quantity}"
        }

    def _check_provider_type_criterion(self, criterion: Dict[str, Any], 
                                     patient_data: Dict[str, Any]) -> tuple[bool, Dict[str, Any]]:
        ...
        return is_met, {
            ...
            'message': f"Provider type acceptable" if is_met 
                      else f"Provider type '{provider_type}' not in allowed list"
        }

    def _generate_criterion_recommendation(self, criterion: Dict[str, Any], 
    ...
    def _generate_alternative_recommendation(self, drug_name: str, 
