...
class GuidelineAgentLogic:
    ...
    def __init__(self, agent_id: str, logger_instance: Optional[logging.Logger] = None):
        ...
        self.guidelines_config = {
            "AHA_HF_2022": {
                ...
                "focus": ["SGLT2 inhibitors", "heart failure", "dapagliflozin", "empagliflozin", "canagliflozin", "ertugliflozin"]
            },
            "ADA_DIABETES_2023": {
                ...
                "focus": ["GLP-1 agonists", "diabetes", "semaglutide", "liraglutide", "dulaglutide"]
            },
            "TEST_GUIDELINE": {
            ...
            with sync_playwright() as p:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    if 'application/pdf' in content_type:
                        ...
                        for selector in ['a[href$=".pdf"]', 'button:has-text("Download")', 'a:has-text("PDF")']:
                            try:
    ...
    def _smart_chunk_text(self, text: str, page_boundaries: List[int], 
    ...
    def _extract_pa_criteria(self, search_results: List[Dict[str, Any]], 
                           drug_name: str, condition: str) -> Dict[str, Any]:
        ...
        for result in search_results[:5]:  # Increased to top 5 results for better coverage
            ...
            if any(keyword in text for keyword in ['recommended', 'should be used', 'indicated for', 'first-line', 'preferred']):
            ...
            if any(keyword in text for keyword in ['prior to', 'before initiating', 'must have', 'requires', 'prerequisite']):
            ...
            if any(keyword in text for keyword in ['dose', 'dosing', 'mg', 'daily', 'twice daily', 'once daily']):
                if not drug_name or drug_name.lower() in text:
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            ...
            results = self._search_guidelines(test_query, n_results=1)
