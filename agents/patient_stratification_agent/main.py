...
class PatientStratificationAgent:
    ...
    async def handle_info_request(self, message: MCPMessage) -> None:
        ...
        if info_type == "general":
            ...
            info = {
                ...
                "cache_hit_rate": self.logic.metrics["cache_hits"] / max(
                    self.logic.metrics["cache_hits"] + self.logic.metrics["cache_misses"], 1
                ),
        ...
        try:
            ...
            while self.running:
                ...
                if int(datetime.now().timestamp()) % 60 == 0:
                    logger.debug(f"Health check - Requests: {self.request_count}, Success rate: {self.success_count/max(self.request_count, 1):.2%}")
    ...
    async def stop(self) -> None:
        ...
        try:
            # Log final metrics
            logger.info(f"Final metrics - Total: {self.request_count}, Success: {self.success_count}, Errors: {self.error_count}")
