...
class WebResearchLogic:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        if self._session is None or self._session.closed:
            timeout = aiohttp.ClientTimeout(total=self.request_timeout, connect=self.connection_timeout, sock_connect=self.connection_timeout, sock_read=self.socket_timeout)
            connector = aiohttp.TCPConnector(limit=10, limit_per_host=5, ttl_dns_cache=300, use_dns_cache=True, enable_cleanup_closed=True, keepalive_timeout=30, timeout_ceil_threshold=5)
            self._session = aiohttp.ClientSession(timeout=timeout, headers=self.session_headers, connector=connector)
    ...
    def _validate_search_parameters(self, params: Dict[str, Any]) -> tuple[bool, Optional[str], str, int]:
        ...
        if drug_name and isinstance(drug_name, str) and drug_name.strip():
            ...
            if len(drug_name) > 200: return False, "drug_name too long (max 200 chars).", "", 0
        ...
        if disease_name and isinstance(disease_name, str) and disease_name.strip():
            ...
            if len(disease_name) > 200: return False, "disease_name too long (max 200 chars).", "", 0
            if pubmed_query_term: pubmed_query_term += " AND "
        ...
        if search_terms and isinstance(search_terms, str) and search_terms.strip():
            ...
            if len(search_terms) > 500: return False, "search_terms too long (max 500 chars).", "", 0
            if pubmed_query_term: pubmed_query_term += " AND "
            pubmed_query_term += f"({search_terms})" # Assume search_terms might already be structured
        ...
        if not pubmed_query_term: # Fallback to raw_query if no structured terms provided
            if raw_query and isinstance(raw_query, str) and raw_query.strip():
                ...
                if len(raw_query) > 1000: return False, "Fallback query too long (max 1000 chars).", "", 0
                pubmed_query_term = raw_query
                self.logger.warning(f"Using raw 'query' parameter for PubMed search: '{raw_query[:100]}...'")
            else:
                return False, "No valid search parameters (drug_name, disease_name, search_terms, or query) provided.", "", 0
        
        try:
            max_results_int = int(max_results) if max_results is not None else 3
        except (ValueError, TypeError): return False, f"max_results must be int, got: {type(max_results).__name__}", "", 0
        if not 1 <= max_results_int <= 50: return False, "max_results must be between 1 and 50", "", 0
    ...
    def _validate_message_structure(self, message_data: Any) -> tuple[bool, Optional[str]]:
        if not isinstance(message_data, dict): return False, f"Message must be dict, got {type(message_data)}"
        if not isinstance(message_data.get("mcp_header"), dict): return False, "Missing or invalid mcp_header"
        if not isinstance(message_data.get("payload"), dict): return False, "Missing or invalid payload"
        return True, None

    def get_capabilities(self) -> List[Dict[str, Any]]:
        return [{"name": "perform_web_search", 
                 ...
                 "features": ["structured_output", "retry_logic", "rate_limiting", "mock_data_support"], 
                 "data_sources": ["PubMed/NCBI E-utils"]
                }]

    def _create_mock_data(self, api_query_term: str, max_results: int, original_input_query: Optional[str]) -> SearchResult:
        self.logger.warning(f"PUBMED_MOCK: Returning mock data for API query term: '{api_query_term[:50]}...' (Original input: '{original_input_query[:50] if original_input_query else 'N/A'}')")
        mock_articles = [
            PubMedArticle(title=f"Mock Study 1 for {api_query_term}", pmid="MOCKPMID1", abstract=f"This is mock abstract 1 for the query: {api_query_term}."),
            PubMedArticle(title=f"Mock Review 2 on {api_query_term}", pmid="MOCKPMID2", abstract=f"A mock review article discussing various aspects of {api_query_term}.")
        ]
        return SearchResult(query_used_for_api=api_query_term, articles=mock_articles[:max_results], total_found_by_esearch=len(mock_articles), search_time_ms=150, source="mock_data", original_input_query=original_input_query)
    ...
    async def _make_ncbi_request(self, url: str, params: Dict[str, str], description: str) -> str:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                self.logger.info(f"NCBI API request (attempt {attempt + 1}/{self.max_retries + 1}): {description}")
                ...
                async with asyncio.timeout(overall_attempt_timeout):
                    async with session.get(url, params=params_to_send) as response:
                        ...
                        if response.status >= 400:
                            error_body = "N/A"; 
                            try: error_body = await response.text(errors='replace')
                            ...
                            if response.status == 400: current_exception = ValueError(err_msg_detail)
                            elif response.status == 401 or response.status == 403: current_exception = PermissionError(err_msg_detail)
                            elif response.status == 404: current_exception = FileNotFoundError(err_msg_detail)
                            elif response.status == 429 or response.status >= 500: current_exception = ConnectionError(err_msg_detail)
                            else: current_exception = ConnectionError(err_msg_detail)
                            last_exception = current_exception
                            if not (response.status == 429 or response.status >= 500): raise current_exception
                        else:
                            content = await response.text()
                            if not content or not content.strip(): raise ValueError(f"Empty response from NCBI API for {description}")
                            self.logger.info(f"NCBI API request successful: {description}")
                            return content
                        
            except asyncio.TimeoutError as e: last_exception = TimeoutError(f"Overall task timeout on attempt {attempt + 1}: {description} - {e}")
            except aiohttp.ClientTimeout as e: last_exception = TimeoutError(f"AIOHTTP ClientTimeout on attempt {attempt + 1}: {description} - {e}")
            except aiohttp.ClientConnectionError as e:
                if "Connection refused" in str(e): last_exception = ConnectionRefusedError(f"Connection refused by NCBI: {description} - {e}")
                else: last_exception = ConnectionError(f"AIOHTTP ClientConnectionError on attempt {attempt + 1}: {description} - {e}")
            except aiohttp.ContentTypeError as e: last_exception = ValueError(f"AIOHTTP ContentTypeError on attempt {attempt + 1}: {description} - {e}"); raise last_exception # Non-retryable
            except Exception as e: last_exception = RuntimeError(f"Unexpected error on attempt {attempt + 1}: {description} - {type(e).__name__} - {e}")
            
            self.logger.error(str(last_exception))
            if attempt < self.max_retries:
                if not isinstance(last_exception, (ConnectionError, TimeoutError, ConnectionRefusedError)):
                    self.logger.error(f"Non-retryable error for {description}. Failing early."); raise last_exception
                delay = self.retry_delay_base * (2 ** attempt)
                self.logger.warning(f"Attempt {attempt+1} for {description} failed. Retrying in {delay}s...")
    ...
    def _parse_article_xml_details(self, pubmed_article_element: ET.Element) -> PubMedArticle:
        ...
        for abstract_text_node in pubmed_article_element.findall(".//Abstract/AbstractText"):
            text_content = "".join(abstract_text_node.itertext()).strip() # Handles mixed content like <i>
            if text_content:
                label = abstract_text_node.get("Label")
                abstract_parts.append(f"**{label.strip()}:** {text_content}" if label else text_content)
        ...
        if author_list_node is not None:
            for author_node in author_list_node.findall(".//Author"):
                ...
                name_parts = []
                if lastname_node is not None and lastname_node.text: name_parts.append(lastname_node.text.strip())
                if forename_node is not None and forename_node.text: name_parts.append(forename_node.text.strip())
                elif initials_node is not None and initials_node.text: name_parts.append(initials_node.text.strip())
                if name_parts: authors_list.append(" ".join(name_parts))
        
        journal_title_node = pubmed_article_element.find(".//Journal/Title")
        journal = journal_title_node.text.strip() if journal_title_node is not None and journal_title_node.text else None
        ...
        return PubMedArticle(
            ...
            journal=journal, publication_date=pub_date_str, doi=doi
        )

    async def _fetch_pubmed_data(self, constructed_api_query_term: str, max_results: int, original_input_query_for_result: Optional[str]) -> SearchResult:
        start_time_ns = time.perf_counter_ns()
        try:
            if self.pubmed_use_mock: 
                return self._create_mock_data(constructed_api_query_term, max_results, original_input_query_for_result)

            self.logger.info(f"Searching PubMed with API term: '{constructed_api_query_term[:100]}...', max_results: {max_results}")
            
            esearch_params = {"db": "pubmed", "term": constructed_api_query_term, "retmax": str(max_results), "usehistory": "y", "retmode": "xml"}
            search_response_xml_str = await self._make_ncbi_request(self.base_url_esearch, esearch_params, f"ESearch for '{constructed_api_query_term[:50]}'")
            
            search_xml_root = ET.fromstring(search_response_xml_str)
            id_list_node = search_xml_root.find("IdList")
            article_ids = [id_node.text for id_node in id_list_node.findall("Id") if id_node.text] if id_list_node is not None else []
            ...
            if not article_ids:
                self.logger.info(f"No PubMed article IDs found for API term: '{constructed_api_query_term}'. Total reported by ESearch: {total_found}")
                articles_data = []
            else:
                self.logger.info(f"Found {len(article_ids)} PubMed IDs (ESearch total: {total_found}). Fetching details...")
                ...
                efetch_params = {"db": "pubmed", "id": ",".join(ids_to_fetch), "rettype": "abstract", "retmode": "xml"}
                fetch_response_xml_str = await self._make_ncbi_request(self.base_url_efetch, efetch_params, f"EFetch for {len(ids_to_fetch)} IDs")
                fetch_xml_root = ET.fromstring(fetch_response_xml_str)
                articles_data = [self._parse_article_xml_details(article_element) for article_element in fetch_xml_root.findall(".//PubmedArticle")]
                articles_data = [article for article in articles_data if article is not None] # Filter out None if parsing failed for an article

            search_time_ms = (time.perf_counter_ns() - start_time_ns) // 1_000_000
            self.logger.info(f"Fetched {len(articles_data)} articles in {search_time_ms}ms for API term '{constructed_api_query_term[:50]}...'.")
            return SearchResult(query_used_for_api=constructed_api_query_term, articles=articles_data, total_found_by_esearch=total_found, search_time_ms=search_time_ms, original_input_query=original_input_query_for_result)
        
        except (TimeoutError, ConnectionError, ConnectionRefusedError, RuntimeError, ValueError, PermissionError, FileNotFoundError) as e:
            ...
            return SearchResult(query_used_for_api=constructed_api_query_term, articles=[], error=error_msg, search_time_ms=(time.perf_counter_ns() - start_time_ns) // 1_000_000, original_input_query=original_input_query_for_result)
    ...
    async def process_message(self, message_data: Dict[str, Any], client: Any) -> Optional[Dict[str, Any]]:
        ...
        if not is_valid_msg: self.logger.error(f"Invalid message structure: {msg_err}"); return None
        ...
        if message_type == MessageType.PONG: self.logger.debug(f"PONG from {sender_id}"); return None
        if message_type == MessageType.DISCOVERY_ACK: self.logger.info(f"Registration confirmed by {sender_id}"); return None
        ...
        else: self.logger.warning(f"Unhandled message type: {message_type.value}"); return None
    ...
    async def _handle_task_assign(self, payload: Dict[str, Any], correlation_id: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            is_valid_params, param_err_msg, constructed_api_query, max_results_validated = \
            ...
            original_input_query = parameters.get("query") if parameters.get("query") != constructed_api_query else None
            if not original_input_query and (parameters.get("drug_name") or parameters.get("disease_name") or parameters.get("search_terms")):
                ...
                if parameters.get("drug_name"): original_input_query_parts.append(f"Drug: {parameters.get('drug_name')}")
                if parameters.get("disease_name"): original_input_query_parts.append(f"Disease: {parameters.get('disease_name')}")
                if parameters.get("search_terms"): original_input_query_parts.append(f"Terms: {parameters.get('search_terms')}")
                original_input_query = "; ".join(original_input_query_parts)


            self.logger.info(f"Performing PubMed search for task {task_id} with API term: '{constructed_api_query[:100]}...', max_results: {max_results_validated}")
            search_result_obj = await self._fetch_pubmed_data(constructed_api_query, max_results_validated, original_input_query)

            if search_result_obj.error:
                self.logger.error(f"PubMed search failed for task {task_id}: {search_result_obj.error}")
                return {"message_type": MessageType.TASK_FAIL.value, "payload": {**response_payload_base, "error_message": search_result_obj.error}}
            ...
            self.logger.info(f"PubMed search completed for task {task_id}: {len(search_result_obj.articles)} articles found (ESearch total: {search_result_obj.total_found_by_esearch}).")
    ...
    def get_status(self) -> Dict[str, Any]:
        return {"agent_id": self.agent_id, "version": self.version, "mock_mode": self.pubmed_use_mock, 
