# C:\Users\rossd\Downloads\RossNetAgents\agents\research\drug_safety_agent\main.py
# Step: Critical-Fixes-DrugSafety-Main-ResponseHandling
...
try:
    ...
    print(f"CRITICAL_ERROR_DRUGSAFETY_MAIN: Failed to import core_infra: {e}")
    sys.exit(1)
...
class DrugSafetyAgentManager:
    ...
    async def handle_incoming_message(self, message: MCPMessage):
        ...
        if not self.drug_safety_logic or not self.mcp_client:
            logger.error("Logic/MCPClient instance missing in DrugSafetyAgent handler")
            return
    ...
    async def initialize_components(self):
        """Initialize DrugSafetyAgentLogic and MCPClient"""
        try:
            ...
            logger.info(
                f"DrugSafetyAgent components initialized (Version: {AGENT_VERSION})"
            )
            ...
            logger.critical(
                f"Failed to initialize DrugSafetyAgent components: {e}", exc_info=True
            )
            return False
...
if __name__ == "__main__":
    try:
        ...
        logger.info("DrugSafetyAgent interrupted")
        sys.exit(0)
    except Exception as e:
