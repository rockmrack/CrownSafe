...
class ClinicalTrialsAgentLogic:
        ...
        return [
            {
                ...
                "output_format": {
                    "query_used_for_api": "string",
                    "trials_count_api": "integer",
                    "trials_returned_batch": "integer",
                    "trials_data": "list_of_objects",
                    "error": "string",
                },
    ...
    def _validate_parameters(
        self, drug_name: Any, disease_name: Any, search_terms: Any, max_trials: Any
    ) -> tuple[bool, Optional[str], int, Dict[str, str]]:
        query_params: Dict[str, str] = {}
        validated_max_trials = 5
        try:
        ...
        drug_name_s = (
            drug_name.strip() if drug_name and isinstance(drug_name, str) else None
        )
        disease_name_s = (
            disease_name.strip()
        ...
        if disease_name_s and len(disease_name_s) > 500:
            return False, "disease_name too long.", validated_max_trials, query_params
        if search_terms_s and len(search_terms_s) > 500:
            return False, "search_terms too long.", validated_max_trials, query_params
        if search_terms_s:
            query_params["query.term"] = search_terms_s
        elif drug_name_s and disease_name_s:
            query_params["query.intr"] = drug_name_s
    ...
    def _make_sync_api_request(
        ...
        for attempt in range(
            self.max_retries + 1
        ):  # Initial attempt + self.max_retries
            try:
                # Rate limiting is handled in the async _fetch_clinical_trials_data before calling this sync method via executor
                self.logger.info(
                    f"ClinicalTrials API request (attempt {attempt + 1}/{self.max_retries + 1}) using 'requests'"
                )
                self.logger.info(f"  Target URL (constructed): {log_url}")
                self.logger.debug(
                    f"  Parameters: {api_params}, Headers: {self.working_headers}"
                )
                ...
                self.logger.debug(
                    f"  Response Headers Received: {dict(response.headers)}"
                )
                ...
                json_response = (
                    response.json()
                )  # Can raise requests.exceptions.JSONDecodeError
                ...
                err_msg_detail = (
                    f"HTTP {e.response.status_code} {e.response.reason} from API. URL: {e.request.url}. "
                ...
                if e.response.status_code == 400:
                    ...
                    current_exception = PermissionError(
                        f"Unauthorized (401): {err_msg_detail}"
                    )
                elif e.response.status_code == 403:
                    current_exception = PermissionError(
                        f"Forbidden (403): {err_msg_detail}"
                    )
                elif e.response.status_code == 404:
                    current_exception = FileNotFoundError(
                        f"Not Found (404): {err_msg_detail}"
                    )
                    ...
                    current_exception = ConnectionError(
                        f"Client Error ({e.response.status_code}): {err_msg_detail}"
                    )  # Other 4xx

                last_exception = current_exception
                if not (
                    e.response.status_code == 429 or e.response.status_code >= 500
                ):  # Don't retry non-server/non-rate-limit client errors
                    raise last_exception  # Fail fast
            except (
                requests.exceptions.ConnectionError
            ) as e:  # Includes DNS, refused, etc.
                ...
                last_exception = ValueError(
                    f"JSONDecodeError on attempt {attempt + 1}: {e}. Response text: {response.text[:200] if 'response' in locals() else 'N/A'}"
                )
                ...
                last_exception = RuntimeError(
                    f"Unexpected error during 'requests' API call attempt {attempt + 1}: {type(e).__name__} - {e}"
                )
                self.logger.error(str(last_exception), exc_info=True)
            ...
            if attempt < self.max_retries:
                ...
                self.logger.warning(
                    f"Request attempt {attempt + 1} failed with retryable error. Retrying in {delay}s... ({str(last_exception)})"
                )
        ...
        critical_fallback_error = (
            "All API request attempts failed after retries (sync)."
        )
    ...
    def _parse_trial_data(self, study_json: Dict[str, Any]) -> Optional[ClinicalTrial]:
        ...
        try:
            protocol_section = study_json.get("protocolSection", {})
            id_module = protocol_section.get("identificationModule", {})
            status_module = protocol_section.get("statusModule", {})
            nct_id = id_module.get("nctId")
            if not nct_id:
                self.logger.warning("Skipping trial due to missing nctId.")
                return None
            ...
            conditions_list = protocol_section.get("conditionsModule", {}).get(
                "conditions", []
            )
            condition_str = "; ".join(conditions_list) if conditions_list else "N/A"
            ...
            self.logger.warning(
                f"Error parsing trial {nct_id if 'nct_id' in locals() else 'Unknown'}: {e}",
                exc_info=True,
            )
            return None

    async def _fetch_clinical_trials_data(
        self, query_params_dict: Dict[str, str], max_trials: int
    ) -> TrialsQueryResult:
        ...
        try:
            self.logger.info(
                f"Fetching Clinical Trials for params: {query_params_str}, max_trials: {max_trials} (using 'requests' via executor)"
            )
            ...
            api_total_count = (
                api_response_json.get("totalCount", 0)
            ...
            self.logger.info(
                f"Successfully fetched/parsed {len(trials_data_parsed)} trials in {search_time_ms}ms. API total: {api_total_count}."
            )
            return TrialsQueryResult(
                query_used_for_api=query_params_str,
                trials_count_api=api_total_count,
                trials_returned_batch=len(trials_data_parsed),
                trials_data=trials_data_parsed,
            ...
            return TrialsQueryResult(
                ...
                trials_data=[],
                error=error_message,
                trials_returned_batch=0,
                trials_count_api=0,
                search_time_ms=search_time_ms_on_error,
            )
    ...
    async def _handle_task_assign(
        ...
        try:
            ...
            disease_name = parameters.get("disease_name")
            search_terms = parameters.get("search_terms")
            max_trials_param = parameters.get("max_trials", 5)
            (
                is_valid_params,
                param_err_msg,
                validated_max_trials,
                ...
                drug_name, disease_name, search_terms, max_trials_param
            )
    ...
    async def shutdown(self):
        ...
        self.logger.info(
            f"ClinicalTrialsAgentLogic shutdown complete for agent {self.agent_id}"
        )
