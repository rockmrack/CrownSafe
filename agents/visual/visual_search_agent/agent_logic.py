        ...
        try:
            ...
            try:
                ...
                if msg.startswith("http_404"):
                    ...
                    return {"status": "FAILED", "error": "image_url_not_found", "error_type": "image_fetch_failed"}
                if msg.startswith("non_image_content"):
                    self.logger.info("Returning image_url_not_image error")
                    return {"status": "FAILED", "error": "image_url_not_image", "error_type": "image_fetch_failed"}
                if msg.startswith("image_too_large"):
                    self.logger.info("Returning image_too_large error")
                    return {"status": "FAILED", "error": "image_too_large", "error_type": "image_fetch_failed"}
                self.logger.info("Returning generic image_fetch_failed error")
                return {"status": "FAILED", "error": "image_fetch_failed", "error_type": "image_fetch_failed"}
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "Analyze the baby product in this image. Identify the product name, brand, and model number if visible. Provide your top 3 most likely matches as a JSON object with a 'suggestions' array, where each object has 'product_name', 'brand', 'model_number', and a 'confidence' score from 0.0 to 1.0. Return ONLY valid JSON without any markdown formatting or code fences."},
                            image_content,
                        ],
                    }
                ],
            ...
            if "401" in error_message and ("api key" in error_message.lower() or "unauthorized" in error_message.lower()):
