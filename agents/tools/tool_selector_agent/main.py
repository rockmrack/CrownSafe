...
AGENT_CAPABILITIES = [
    {"name": "tool_selection", "description": "Selects the appropriate tool or agent for a given subtask."},
    {"name": "capability_matching", "description": "Matches task requirements to registered agent capabilities."},
...
async def handle_incoming_message(client_ref: MCPClient, message: MCPMessage):
    ...
    if tool_selector_logic_instance:
        message_type = message.mcp_header.message_type if hasattr(message, 'mcp_header') else message.get('mcp_header', {}).get('message_type')
        ...
        try:
            ...
            if response_payload:
                ...
                if correlation_id and original_requester_id:
                    ...
                    logger.warning(f"Cannot send {response_type}: Missing correlation_id or original_requester_id in incoming message payload.")
