...
class ToolSelectorLogic:
    ...
    async def _handle_selection_request(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if not required_capabilities and task_description:
            self.logger.warning(
                f"Selection request {correlation_id} received task_description but not required_capabilities. LLM interpretation needed (Not Implemented)."
            )
        ...
        if not isinstance(required_capabilities, list) or not required_capabilities:
            self.logger.error(
                f"Received selection request {correlation_id} with invalid or empty 'required_capabilities'."
            )
            return self._create_failure_response(
                correlation_id,
                original_requester_id,
                "Invalid or empty 'required_capabilities' provided.",
            )

        self.logger.info(
            f"Received tool selection request {correlation_id} for capabilities: {required_capabilities} from {original_requester_id}."
        )
        ...
        # Initiate discovery
        self.logger.info(
            f"Initiating discovery for selection request {correlation_id}. Capabilities: {required_capabilities}"
        )
        ...
        try:
            ...
            self.logger.error(
                f"Failed to send DISCOVERY_QUERY for selection request {correlation_id}: {e}",
                exc_info=True,
            )
    ...
    async def _handle_discovery_result(self, discovery_payload: Dict[str, Any]):
        ...
        self.logger.info(
            f"Handling DISCOVERY_RESPONSE for capabilities: {required_capabilities}. Status: {status}. Found: {len(found_agents)} agents."
        )
        ...
        for req_id, req_state in self.pending_selections.items():
            ...
            if (
                req_state["status"] == "PENDING_DISCOVERY"
                and req_state["required_capabilities"] == required_capabilities
            ):
        ...
        if not target_request_id:
            self.logger.warning(
                f"Received DISCOVERY_RESPONSE for capabilities {required_capabilities}, but no matching selection request found in pending state."
            )
        ...
        if status == "success" and found_agents:
            ...
            if selected_agent_id:
                self.logger.info(
                    f"Discovery successful for selection request {target_request_id}. Selected agent: {selected_agent_id}"
                )
            else:
                self.logger.warning(
                    f"Discovery for selection request {target_request_id} found agents, but none were ACTIVE."
                )
        ...
        if response_payload:
            ...
            try:
                ...
                self.logger.info(
                    f"Sent selection {response_type} response for {target_request_id} to {original_requester_id}."
                )
            except Exception as e:
                self.logger.error(
    ...
    def _create_failure_response(
        self, correlation_id: str, original_requester_id: str, error_message: str
    ) -> Dict[str, Any]:
