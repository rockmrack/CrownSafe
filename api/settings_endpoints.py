...
async def get_settings(
    ...
    logger.info(f"Settings retrieved", extra={
...
async def update_settings(
    ...
    logger.info(f"Settings updated", extra={
        "identifier": identifier[:8] if len(identifier) > 8 else identifier,
...
async def toggle_crashlytics(
    ...
    **Privacy Note**: 
    ...
    try:
        ...
        # Log the change
        logger.info(f"Crashlytics toggled", extra={
            "identifier": identifier[:8] if len(identifier) > 8 else identifier,
        ...
        return {
            ...
            "privacy_note": "Crash reports contain no personal information" if toggle_request.enabled else "No crash data will be collected",
            "trace_id": trace_id
        }
        
    except Exception as e:
        logger.error(f"Failed to toggle Crashlytics: {e}", extra={
            "trace_id": trace_id
        })
        raise HTTPException(
    ...
    return {
        ...
        "privacy_info": {
            ...
            "data_collected": [
                ...
                "app version",
                "crash timestamp"
            ] if settings.crashlytics_enabled else [],
            "personal_data": False,
            "can_disable": True
        }
    }
...
async def reset_settings(
    ...
    logger.info(f"Settings reset to defaults", extra={
        "identifier": identifier[:8] if len(identifier) > 8 else identifier,
        "trace_id": trace_id
    })
    ...
    return SettingsResponse(
        ok=True,
        settings=settings_store[identifier],
        updated_at=datetime.utcnow()
    )
