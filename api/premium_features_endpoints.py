...
async def check_pregnancy_safety(
    ...
    try:
        ...
        if payload.user_id and payload.user_id != user_id:
            raise HTTPException(status_code=403, detail={"success": False, "error": {"message": "Mismatched user_id vs token"}})
        logger.info(f"Pregnancy safety check for user {user_id}, trimester {payload.trimester}")
        
        # Validate user exists (defensive)
        user = db.query(User).filter(User.id == user_id).first()
        if not user:
            raise HTTPException(status_code=404, detail={"success": False, "error": {"message": "User not found"}})
        ...
        if result.get("alerts"):
            for alert in result["alerts"]:
                if alert.get("risk_level") == "High":
                    recommendations.append(f"AVOID: {alert['ingredient']} - {alert.get('reason', 'High risk during pregnancy')}")
                elif alert.get("risk_level") == "Moderate":
                    recommendations.append(f"CAUTION: {alert['ingredient']} - {alert.get('reason', 'Use with caution')}")
...
async def check_pregnancy_safety_dev(
    ...
    try:
        ...
        if not dev_entitled(payload.user_id or 0, REQUIRED_FEATURE):
            raise HTTPException(status_code=402, detail="Subscription required for pregnancy safety check")
...
async def comprehensive_safety_check(
    request: CombinedSafetyCheckRequest,
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        if not dev_entitled(request.user_id, REQUIRED_FEATURE):
            ...
            if not getattr(user, "is_subscribed", False):
                raise HTTPException(status_code=402, detail="Subscription required for comprehensive safety check")
        else:
