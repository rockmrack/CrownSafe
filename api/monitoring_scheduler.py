...
class ProductMonitoringScheduler:
        ...
        try:
            ...
            if product.upc_code:
                recalls = db.query(RecallDB).filter(
                    RecallDB.upc_codes.contains([product.upc_code])
                ).all()
    ...
    async def run_monitoring_cycle(cls) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Monitoring run {run_id} completed: {products_checked} checked, {new_recalls_found} new recalls, {notifications_sent} notifications")
    ...
    async def auto_add_from_scans(cls):
        ...
        try:
            with get_db_session() as db:
                ...
                recent_scans = db.query(ImageJob).join(ImageExtraction).filter(
