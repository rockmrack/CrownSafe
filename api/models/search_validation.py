...
class SecureAdvancedSearchRequest(BaseModel):
    ...
    riskCategory: Optional[Literal[
        "drug", "device", "food", "cosmetic", 
        "supplement", "toy", "baby_product", "other"
    ]] = Field(
        None,
        description="Risk category filter",
        examples=["toy"]
    )
    ...
    date_from: Optional[date] = Field(
        None,
        description="Start date for recall date range",
        examples=["2024-01-01"]
    )
    
    date_to: Optional[date] = Field(
        None,
        description="End date for recall date range",
        examples=["2024-12-31"]
    )
    ...
    def validate_agencies(cls, v: Optional[List[str]]) -> Optional[List[str]]:
        ...
        valid_agencies = {
            "FDA", "CPSC", "NHTSA", "USDA", "EPA",
            "FSIS", "CDC", "EU_SAFETY_GATE", "UK_OPSS",
    ...
    def validate_search_criteria(self):
        ...
        if not (has_text_search or has_filters):
            raise ValueError(
                ...
                "query, product, keywords, or filters"
            )
