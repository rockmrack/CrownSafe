...
AGENCIES = [
    ...
    Agency(code="FDA", name="U.S. Food and Drug Administration", country="United States", website="https://www.fda.gov"),
    Agency(code="CPSC", name="U.S. Consumer Product Safety Commission", country="United States", website="https://www.cpsc.gov"),
    Agency(code="NHTSA", name="National Highway Traffic Safety Administration", country="United States", website="https://www.nhtsa.gov"),
    Agency(code="USDA_FSIS", name="Food Safety and Inspection Service", country="United States", website="https://www.fsis.usda.gov"),
    
    # Canada (3 agencies)
    Agency(code="HEALTH_CANADA", name="Health Canada", country="Canada", website="https://www.canada.ca/en/health-canada"),
    Agency(code="CFIA", name="Canadian Food Inspection Agency", country="Canada", website="https://www.inspection.gc.ca"),
    Agency(code="TRANSPORT_CANADA", name="Transport Canada", country="Canada", website="https://www.tc.gc.ca"),
    
    # Mexico (2 agencies)
    Agency(code="PROFECO", name="Federal Consumer Protection Agency", country="Mexico", website="https://www.profeco.gob.mx"),
    Agency(code="COFEPRIS", name="Federal Commission for Protection against Sanitary Risk", country="Mexico", website="https://www.gob.mx/cofepris"),
    
    # Europe (22 agencies)
    # European Union (1 agency)
    Agency(code="EU_SAFETY_GATE", name="EU Safety Gate (RAPEX)", country="European Union", website="https://ec.europa.eu/safety-gate"),
    
    # United Kingdom (2 agencies)
    Agency(code="UK_OPSS", name="UK Office for Product Safety and Standards", country="United Kingdom", website="https://www.gov.uk/government/organisations/office-for-product-safety-and-standards"),
    Agency(code="UK_FSA", name="Food Standards Agency", country="United Kingdom", website="https://www.food.gov.uk"),
    
    # France (1 agency)
    Agency(code="RAPPEL_CONSO", name="RappelConso", country="France", website="https://rappel.conso.gouv.fr"),
    
    # Germany (1 agency)
    Agency(code="LEBENSMITTELWARNUNG", name="Lebensmittelwarnung", country="Germany", website="https://www.lebensmittelwarnung.de"),
    
    # Netherlands (1 agency)
    Agency(code="NVWA", name="Food and Product Safety Authority", country="Netherlands", website="https://www.nvwa.nl"),
    
    # Spain (1 agency)
    Agency(code="AESAN", name="Food Safety and Nutrition Agency", country="Spain", website="https://www.aesan.gob.es"),
    
    # Italy (1 agency)
    Agency(code="MINISTRY_HEALTH_ITALY", name="Ministry of Health", country="Italy", website="https://www.salute.gov.it"),
    
    # Switzerland (3 agencies)
    Agency(code="FCAB", name="Federal Consumer Affairs Bureau", country="Switzerland", website="https://www.konsum.admin.ch"),
    Agency(code="FSVO", name="Federal Food Safety and Veterinary Office", country="Switzerland", website="https://www.blv.admin.ch"),
    Agency(code="SWISSMEDIC", name="Swissmedic", country="Switzerland", website="https://www.swissmedic.ch"),
    
    # Sweden (2 agencies)
    Agency(code="CONSUMER_AGENCY_SWEDEN", name="Consumer Agency", country="Sweden", website="https://www.konsumentverket.se"),
    Agency(code="FOOD_AGENCY_SWEDEN", name="Food Agency", country="Sweden", website="https://www.livsmedelsverket.se"),
    
    # Norway (2 agencies)
    Agency(code="DSB", name="Directorate for Civil Protection", country="Norway", website="https://www.dsb.no"),
    Agency(code="MATTILSYNET", name="Food Safety Authority", country="Norway", website="https://www.mattilsynet.no"),
    
    # Denmark (2 agencies)
    Agency(code="SAFETY_TECH_AUTHORITY_DENMARK", name="Safety Technology Authority", country="Denmark", website="https://www.sik.dk"),
    Agency(code="FOOD_ADMIN_DENMARK", name="Food Administration", country="Denmark", website="https://www.foedevarestyrelsen.dk"),
    
    # Finland (2 agencies)
    Agency(code="TUKES", name="Safety and Chemicals Agency", country="Finland", website="https://www.tukes.fi"),
    Agency(code="FOOD_AUTHORITY_FINLAND", name="Food Authority", country="Finland", website="https://www.ruokavirasto.fi"),
    
    # Asia-Pacific (7 agencies)
    # Singapore (1 agency)
    Agency(code="CPSO", name="Consumer Product Safety Office", country="Singapore", website="https://www.cpsa.gov.sg"),
    
    # Australia (3 agencies)
    Agency(code="ACCC", name="Competition and Consumer Commission", country="Australia", website="https://www.accc.gov.au"),
    Agency(code="TGA", name="Therapeutic Goods Administration", country="Australia", website="https://www.tga.gov.au"),
    Agency(code="FSANZ", name="Food Standards (shared with NZ)", country="Australia", website="https://www.foodstandards.gov.au"),
    
    # New Zealand (3 agencies)
    Agency(code="TRADING_STANDARDS_NZ", name="Trading Standards", country="New Zealand", website="https://www.consumerprotection.govt.nz"),
    Agency(code="MPI", name="Ministry for Primary Industries", country="New Zealand", website="https://www.mpi.govt.nz"),
    Agency(code="MEDSAFE", name="Medsafe", country="New Zealand", website="https://www.medsafe.govt.nz"),
    
    # South America (4 agencies)
    # Brazil (3 agencies)
    Agency(code="ANVISA", name="National Health Surveillance Agency", country="Brazil", website="https://www.gov.br/anvisa"),
    Agency(code="SENACON", name="National Consumer Secretary", country="Brazil", website="https://www.gov.br/mj/pt-br/assuntos/senacon"),
    Agency(code="INMETRO", name="National Institute of Metrology", country="Brazil", website="https://www.gov.br/inmetro"),
    
    # Argentina (1 agency)
    Agency(code="ANMAT", name="National Administration of Drugs, Foods and Medical Technology", country="Argentina", website="https://www.anmat.gov.ar")
]
...
def convert_recall_to_safety_issue(recall: RecallDB, agency_code: str) -> SafetyIssue:
    """Convert RecallDB object to SafetyIssue model with safe null handling"""
    try:
        # Determine severity based on hazard field (safe null handling)
        severity = "medium"
        ...
        # Build title (safe null handling)
        product_name = getattr(recall, 'product_name', None) or "Product Recall"
        ...
        # Handle recall date safely
        recall_date = getattr(recall, 'recall_date', None)
        recall_date_str = None
        status = "open"
        if recall_date:
            try:
        ...
        if isinstance(regions_affected, list):
            affected_countries = regions_affected
        elif country:
        ...
        return SafetyIssue(
            ...
            title=title[:500] if title else "Product Recall",  # Limit title length
            description=getattr(recall, 'description', None),
            productName=product_name,
            brand=brand,
            model=getattr(recall, 'model_number', None),
            upc=getattr(recall, 'upc', None),
            hazard=hazard,
            riskCategory=getattr(recall, 'hazard_category', None),
            severity=severity,
            status=status,
            imageUrl=None,  # We don't store images in the database yet
            affectedCountries=affected_countries,
            recallDate=recall_date_str,
            lastUpdated=datetime.now().isoformat(),
            sourceUrl=source_url
        )
    except Exception as e:
        logger.error(f"Error converting recall to safety issue: {e}", exc_info=True)
        # Return minimal valid object on error
        return SafetyIssue(
            id=str(getattr(recall, 'id', 'unknown')),
            agencyCode=agency_code,
            ...
            recallDate=None,
            lastUpdated=datetime.now().isoformat(),
            sourceUrl="https://babyshield.cureviax.ai"
        )
    ...
    try:
        ...
        if connector:
            ...
            for recall in recalls[:limit]:  # Limit results
                if (product.lower() in recall.product_name.lower() if recall.product_name else False) or \
                   (product.lower() in recall.description.lower() if recall.description else False) or \
                   (product.lower() in (recall.brand or "").lower()):
...
async def search_agency_recalls(
    ...
    try:
        with get_db_session() as db:
            ...
            # Build query
            query = db.query(RecallDB).filter(
                RecallDB.source_agency == internal_agency
            )
            
            # Add product search
            search_conditions = or_(
                RecallDB.product_name.ilike(f"%{product}%"),
                RecallDB.brand.ilike(f"%{product}%"),
                RecallDB.description.ilike(f"%{product}%"),
                RecallDB.model_number.ilike(f"%{product}%")
            )
            ...
            # Order by recall_id for consistent pagination
            query = query.order_by(RecallDB.recall_id)
            
            # Fetch limit + 1 to check if there are more results
            recalls = query.limit(limit + 1).all()
            
            # Check if there are more results
            has_more = len(recalls) > limit
            if has_more:
                recalls = recalls[:limit]  # Trim to requested limit
                next_cursor = recalls[-1].recall_id if recalls else None
            else:
                next_cursor = None
            
            # Convert to SafetyIssue objects (handle empty results and conversion errors)
            items = []
            ...
            try:
                total_query = db.query(RecallDB).filter(
                    RecallDB.source_agency == internal_agency
