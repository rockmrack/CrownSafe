...
async def check_entitlement(
    ...
    try:
        ...
        known_features = {
            "safety.check", "safety.comprehensive", "premium.pregnancy", 
            "premium.allergy", "enhanced.scan", "basic.scan"
        }
    ...
    try:
        ...
        plans = [
            SubscriptionPlan(
                ...
                price=4.99,
                features=[
                    PlanFeature(name="safety.check", description="Basic product safety checking", enabled=True),
                    PlanFeature(name="basic.scan", description="Standard barcode scanning", enabled=True),
                ],
                popular=False,
                trial_days=7
            ),
            SubscriptionPlan(
                ...
                features=[
                    PlanFeature(name="safety.check", description="Basic product safety checking", enabled=True),
                    PlanFeature(name="safety.comprehensive", description="Comprehensive safety analysis", enabled=True),
                    PlanFeature(name="premium.pregnancy", description="Pregnancy safety checks", enabled=True),
                    PlanFeature(name="premium.allergy", description="Allergy sensitivity checks", enabled=True),
                    PlanFeature(name="enhanced.scan", description="Enhanced barcode scanning", enabled=True),
                ],
                popular=True,
                trial_days=14
            ),
            SubscriptionPlan(
                ...
                features=[
                    ...
                    PlanFeature(name="premium.allergy", description="Allergy sensitivity checks", enabled=True),
                    PlanFeature(name="enhanced.scan", description="Enhanced barcode scanning", enabled=True),
                    PlanFeature(name="family.profiles", description="Multiple family member profiles", enabled=True),
                ],
                popular=False,
                trial_days=30
            )
        ...
        if feature:
            plans = [
                ...
                if any(f.name == feature for f in plan.features)
            ]
