...
from api.schemas.tools import (
    PregnancyCheckIn, PregnancyCheckOut, RiskItem,
    AllergyCheckIn, AllergyCheckOut, AllergyHit,
...
def pregnancy_adapter(scan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if raw_result and raw_result.get("unsafe_ingredients"):
        for ingredient, details in raw_result["unsafe_ingredients"].items():
            risks.append(RiskItem(
                ...
                reason=details.get("reason", f"Contains {ingredient} which may pose pregnancy risks"),
    ...
    out = PregnancyCheckOut(
        risks=risks,
        notes=raw_result.get("notes") if raw_result else "Check product labeling for pasteurisation and safety warnings"
    )
...
def allergy_adapter(scan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out = AllergyCheckOut(
        hits=hits,
        summary=raw_result.get("summary") if raw_result else None
    )
...
def _check_pregnancy_safety_from_scan(agent: PregnancyProductSafetyAgent, scan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        pregnancy_unsafe = {
            "alcohol": {"reason": "Alcohol consumption during pregnancy can cause birth defects", "severity": "high"},
            "caffeine": {"reason": "High caffeine intake should be limited during pregnancy", "severity": "low"},
            "raw_milk": {"reason": "Raw milk may contain harmful bacteria", "severity": "high"},
            "unpasteurized": {"reason": "Unpasteurized products may contain listeria", "severity": "moderate"},
            "mercury": {"reason": "Mercury can harm fetal development", "severity": "high"},
            "high_sodium": {"reason": "Excessive sodium can contribute to pregnancy complications", "severity": "low"}
        }
        ...
        return {
            ...
            "notes": "Always consult with healthcare provider for pregnancy-specific dietary advice" if unsafe_ingredients else None
        }
...
def recall_details_adapter(scan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with get_db_session() as db:
        ...
        if conditions:
            ...
            for r in results:
                recalls_data.append(RecallRecord(
