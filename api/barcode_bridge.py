...
def search_exact_barcode(barcode: str, db: Session) -> List[RecallDB]:
    ...
    recalls = db.query(RecallDB).filter(
        or_(
...
async def scan_barcode(
    ...
    try:
        ...
        err = {"ok": False, "match_status": "error", "message": f"Error processing barcode: {str(e)[:100]}"}
            ...
            for recall in exact_recalls[:5]:  # Limit to top 5
                try:
                    recalls.append(RecallMatch(
                        ...
                        recall_date=str(recall.recall_date) if hasattr(recall, 'recall_date') and recall.recall_date else None,
    ...
    # Log scan
    try:
        logger.info(f"Barcode scan completed", extra={
