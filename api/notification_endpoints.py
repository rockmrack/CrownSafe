...
class DeviceToken(Base):
    ...
    notification_types = Column(JSON, default=lambda: {
        "recalls": True,
        "safety_alerts": True,
...
async def unregister_device_dev(
    token: str
):
    """
...
async def update_preferences_dev(
    request: dict
):
    """
...
async def send_test_notification_dev(
    request: dict
):
    """
...
async def get_notification_history(
    ...
    try:
        ...
        notifications = query.order_by(desc(NotificationHistory.sent_at))\
        ...
        # Count unread
        unread_count = db.query(NotificationHistory).filter(
            NotificationHistory.user_id == current_user.id,
