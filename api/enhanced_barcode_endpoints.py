...
async def exact_barcode_scan(
    ...
    try:
        ...
        response_data = {
            ...
            "validation_result": scan_result.barcode_validation.validation_result.value if scan_result.barcode_validation.validation_result else "unknown",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Enhanced scan failed: {str(e)}"
        )
...
async def validate_barcode_format(
    ...
    try:
        ...
        response_data = {
            ...
            "validation_result": validation_result.validation_result.value if validation_result.validation_result else "unknown",
        ...
        if request.expected_type:
            ...
            if validation_result.barcode_type != expected_type:
                response_data["error_message"] = f"Expected {expected_type.value}, got {validation_result.barcode_type.value}"
...
async def _basic_scan_fallback(request: EnhancedScanRequest) -> EnhancedScanResponse:
    ...
    try:
        ...
        return EnhancedScanResponse(
            ...
            validation_details=enhanced_barcode_service.validator.get_validation_summary(validation_result) if request.include_validation_details else None,
            matches=[]
        )
