...
class IncidentAnalyzer:
    ...
    def _find_similar_incidents(
        ...
        similar = db.query(IncidentReport).filter(
            and_(
    ...
    def _find_or_create_cluster(
        ...
        for similar in similar_incidents:
            if similar.cluster_id:
                cluster = db.query(IncidentCluster).filter(
                    IncidentCluster.id == similar.cluster_id
                ).first()
                if cluster:
                    return cluster
    ...
    def _update_cluster_stats(
        ...
        # Check if trending
        recent_count = db.query(IncidentReport).filter(
            and_(
...
async def submit_incident_json(
    ...
    try:
        ...
        if request.product_barcode:
            ...
            product = db.query(RecallDB).filter(
                RecallDB.barcode == request.product_barcode
            ).first()
...
async def get_incident_statistics(
    ...
    # Total incidents
    total_incidents = db.query(IncidentReport).filter(
        IncidentReport.created_at >= cutoff_date
    ).count()
    ...
    severity_stats = db.query(
        ...
        IncidentReport.created_at >= cutoff_date
    ).group_by(IncidentReport.severity_level).all()
    ...
    return ApiResponse(
        ...
        data={
            ...
            "severity_breakdown": {
                s.value: count for s, count in severity_stats
            },
            "type_breakdown": {
                t.value: count for t, count in type_stats
            },
            "active_clusters": active_clusters,
            "agencies_notified": db.query(AgencyNotification).filter(
                AgencyNotification.sent_at >= cutoff_date
            ).count()
        }
    )
...
async def get_incident_statistics_dev(
    days: int = 30
):
    """
