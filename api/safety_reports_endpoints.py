...
class SafetyReportRequest(BaseModel):
    ...
    report_type: str = Field("90_day", description="Type of report (90_day, 30_day, weekly, quarterly_nursery)")
...
async def generate_90_day_report_dev(
    request: SafetyReportRequest
) -> ApiResponse:
    """
    ...
    try:
        ...
        if not dev_entitled(request.user_id, REQUIRED_FEATURE):
            raise HTTPException(status_code=402, detail="Subscription required for 90-day report generation")
...
async def generate_90_day_report(
    ...
    try:
        ...
        ).order_by(desc(ScanHistory.scan_timestamp)).all()
        ...
        for key, group in product_groups.items():
            ...
            product_summaries.append(ProductScanSummary(
        ...
        if request.generate_pdf:
            ...
            try:
                report_agent = ReportBuilderAgentLogic(
                    agent_id=f"report_builder_{report_id}",
                    version="2.1"
                )
...
async def get_my_reports(
    ...
    try:
        ...
        ).order_by(desc(SafetyReport.generated_at)).limit(limit).all()
...
async def get_report_details_dev(
    report_id: str,
    user_id: int
) -> ApiResponse:
    """
...
async def get_report_details(
    ...
    try:
        report = db.query(SafetyReport).filter(
            SafetyReport.report_id == report_id
        ).first()
...
async def generate_quarterly_nursery_report_dev(
    ...
    try:
        ...
        if not dev_entitled(request.user_id, REQUIRED_FEATURE):
            raise HTTPException(status_code=402, detail="Subscription required for quarterly nursery report generation")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate quarterly nursery report: {str(e)}")
            ...
            if category_data["products"]:
                ...
                for product_key, product_group in product_groups.items():
                    ...
                    if product_in_category:
                        ...
                        cat_products.append(ProductScanSummary(
                ...
                category_summaries.append(NurseryProductCategory(
