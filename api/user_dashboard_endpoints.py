...
async def get_dashboard_overview(
    ...
    try:
        ...
        # Get scan statistics
        total_scans = db.query(ImageJob).filter(
            ImageJob.user_id == current_user.id
        ).count()
        
        scans_this_month = db.query(ImageJob).filter(
            ImageJob.user_id == current_user.id,
        ...
        notifications_received = db.query(NotificationHistory).filter(
...
async def get_activity_timeline(
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            scans = db.query(ImageJob).filter(
                ImageJob.user_id == current_user.id,
            ...
            recalls = db.query(MonitoredProduct).filter(
                MonitoredProduct.user_id == current_user.id,
...
async def get_product_categories(
    ...
    try:
        ...
        extractions = db.query(ImageExtraction).join(ImageJob).filter(
            ImageJob.user_id == current_user.id
        ).all()
        ...
        for cat, count in sorted(categories.items(), key=lambda x: x[1], reverse=True):
            ...
            category_stats.append(ProductCategory(
                category=cat,
                count=count,
        ...
        return ok({
            "categories": category_stats[:10],  # Top 10 categories
...
async def get_safety_insights(
    ...
    try:
        ...
        last_scan = db.query(ImageJob).filter(
            ImageJob.user_id == current_user.id
...
async def get_user_achievements(
    ...
    try:
        ...
        # Safety achievements
        recalled_handled = db.query(NotificationHistory).filter(
            NotificationHistory.user_id == current_user.id,
            NotificationHistory.type == "recall",
