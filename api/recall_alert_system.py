...
async def send_test_recall_alert_dev(
    request: dict
):
    """
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to send recall alert: {str(e)}"
        }
...
async def update_alert_preferences_dev(
    request: dict
):
    """
...
async def get_alert_history_dev(
    user_id: int
):
    """
...
class RecallAlertService:
    ...
    async def find_affected_users(
        ...
        try:
            ...
            matching_scans = db.query(ScanHistory).filter(
                or_(
            ...
            ).distinct(ScanHistory.user_id).all()
            ...
            for recall in all_new_recalls:
                ...
                try:
                    new_recall = RecallDB(
                        ...
                        if recall.get("date") else datetime.utcnow()
                    )
                    db.add(new_recall)
                    db.commit()
                except Exception as e:
...
async def test_recall_alert(
    ...
    success = await RecallAlertService.send_recall_alert(
        user_id, mock_recall, db
    )
...
async def check_recalls_now(
    ...
    return {
        "success": True,
        "message": "Recall check initiated in background"
    }
