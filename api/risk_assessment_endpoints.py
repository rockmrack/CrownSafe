...
from core_infra.risk_assessment_models import (
    ProductGoldenRecord, ProductRiskProfile, SafetyIncident,
    CompanyComplianceProfile, RiskAssessmentReport, DataIngestionJob,
...
from core_infra.safety_data_connectors import (
    CPSCDataConnector, EUSafetyGateConnector, CommercialDatabaseConnector,
    DataUnificationEngine, SafetyDataRecord
)
...
async def assess_product_risk(
    ...
    try:
        ...
        incidents = db.query(SafetyIncident).filter(
            SafetyIncident.product_id == product.id
        ).all()
        
        # Step 3: Get company profile
        company_profile = None
        if product.manufacturer:
            company_profile = db.query(CompanyComplianceProfile).filter(
        ...
        risk_profile = db.query(ProductRiskProfile).filter(
            ProductRiskProfile.product_id == product.id
        ).first()
        ...
        if request.include_report:
            report_data = report_generator.generate_report(
                product, risk_profile, risk_components,
                incidents, company_profile, request.report_format
            )
        ...
        return RiskAssessmentResponse(
            ...
            assessment_id=f"ASM-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}",
        ...
        return RiskAssessmentResponse(
            ...
            assessment_id=f"ERROR-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}",
            timestamp=datetime.utcnow()
        )
        ...
        if not scan_result.codes:
            return JSONResponse(
                ...
                content={
                    ...
                    "job_id": f"IMG-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}"
                }
            )
...
async def get_risk_profile(
    ...
    try:
        # Get product
        product = db.query(ProductGoldenRecord).filter(
            ProductGoldenRecord.id == product_id
        ).first()
        ...
        # Get risk profile
        risk_profile = db.query(ProductRiskProfile).filter(
            ProductRiskProfile.product_id == product_id
        ).first()
...
async def get_report(
    ...
    try:
        ...
        report = db.query(RiskAssessmentReport).filter(
            RiskAssessmentReport.id == report_id
        ).first()
        ...
        if report.report_url:
            ...
            # Regenerate report
            product = db.query(ProductGoldenRecord).filter(
                ProductGoldenRecord.id == report.product_id
            ).first()
...
async def get_risk_statistics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        risk_levels = db.query(
            ProductRiskProfile.risk_level,
            func.count(ProductRiskProfile.id)
        ).group_by(ProductRiskProfile.risk_level).all()
        
        stats["risk_distribution"] = {level: count for level, count in risk_levels}
        
        # Recent high-risk products
        high_risk = db.query(ProductRiskProfile).filter(
            ProductRiskProfile.risk_level.in_(["high", "critical"])
        ).order_by(ProductRiskProfile.last_calculated.desc()).limit(5).all()
        ...
        source_counts = db.query(
            ProductDataSource.source_type,
            func.count(ProductDataSource.id)
        ).group_by(ProductDataSource.source_type).all()
...
async def _find_or_create_product(request: RiskAssessmentRequest, db: Session) -> Optional[ProductGoldenRecord]:
    ...
    if request.gtin:
        product = db.query(ProductGoldenRecord).filter(
            ProductGoldenRecord.gtin == request.gtin
        ).first()
        if product:
            return product
    ...
    if request.upc:
        product = db.query(ProductGoldenRecord).filter(
            ProductGoldenRecord.upc == request.upc
        ).first()
        if product:
            return product
...
async def ingest_from_source(source: str, job_id: str, start_date: Optional[datetime], end_date: Optional[datetime]):
    """
    ...
    try:
        if source == "CPSC":
            ...
            records = await connector.fetch_recalls(start_date, end_date)
            # Process records...
            
        elif source == "EU_SAFETY_GATE":
            connector = EUSafetyGateConnector()
            records = await connector.fetch_alerts(start_date)
            # Process records...
