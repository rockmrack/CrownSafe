...
@router.post("/data/export", 
            ...
            dependencies=[Depends(dsar_limiter)])
@privacy_audit_log
async def request_data_export(
    ...
    try:
        ...
        existing = db.query(PrivacyRequest).filter(
            and_(
...
async def request_data_rectification(
    ...
    return create_response(
        format_dsar_response("rectify", "queued", body.jurisdiction or "other"),
        request
    )
...
async def object_to_processing(
    ...
    """
    return create_response(
        format_dsar_response("object", "queued", body.jurisdiction or "other"),
        request
    )
