...
class UpdatePrivacyRequest(BaseModel):
    ...
    export_url: Optional[str] = Field(
        None,
        description="Export download URL (if status=done for export requests)"
    )
...
class PrivacyRequestFilter(BaseModel):
    ...
    kind: Optional[str] = Field(
        None,
        pattern="^(export|delete|rectify|access|restrict|object)$"
    )
    status: Optional[str] = Field(
        None,
        pattern="^(queued|verifying|processing|done|rejected|expired|cancelled)$"
    )
...
async def privacy_request_statistics(
    ...
    try:
        ...
        # Overall statistics
        total_requests = db.query(func.count(PrivacyRequest.id)).filter(
        ...
        # Overdue requests
        now = datetime.now(timezone.utc)
        overdue_count = db.query(func.count(PrivacyRequest.id)).filter(
            and_(
                PrivacyRequest.status.in_(["queued", "verifying", "processing"]),
        ...
        stats = {
            ...
            "overview": {
                ...
                "pending": db.query(func.count(PrivacyRequest.id)).filter(
                    PrivacyRequest.status.in_(["queued", "verifying", "processing"])
                ).scalar() or 0,
                "completed": db.query(func.count(PrivacyRequest.id)).filter(
                    PrivacyRequest.status == "done"
                ).scalar() or 0,
                "rejected": db.query(func.count(PrivacyRequest.id)).filter(
