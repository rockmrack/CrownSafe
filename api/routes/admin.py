...
async def data_freshness(
    ...
    try:
        ...
        latest_update = db.query(
            func.max(EnhancedRecallDB.last_updated)
        ).scalar()
        ...
        for stat in agency_stats:
            agencies.append({
                ...
                "staleness": "fresh" if stat.new_24h > 0 else (
                    "recent" if stat.new_7d > 0 else "stale"
                )
            })
        
        # Get recent ingestion runs
        recent_runs = db.query(IngestionRun).filter(
            IngestionRun.finished_at >= datetime.utcnow() - timedelta(hours=24)
        ).order_by(
...
async def admin_statistics(
    ...
    try:
        ...
        ingestion_stats = db.query(
            func.count(IngestionRun.id).label("total"),
            func.sum(
                func.cast(IngestionRun.status == "success", Integer)
            ).label("success"),
            func.sum(
                func.cast(IngestionRun.status == "failed", Integer)
            ).label("failed"),
            func.avg(
                func.extract("epoch", IngestionRun.finished_at - IngestionRun.started_at)
            ).label("avg_duration")
        ).filter(
            IngestionRun.created_at >= week_ago
        ).first()
