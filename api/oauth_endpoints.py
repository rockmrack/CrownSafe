    ...
    **Privacy Note**: We only store the provider's subject ID and an internal user ID.
    ...
    try:
        ...
        user = db.query(User).filter(
            User.provider_id == provider_hash
        ).first()
        ...
        if not user:
            ...
            logger.info(f"New OAuth user created", extra={
            ...
            logger.info(f"OAuth user login", extra={
                "provider": login_data.provider,
                "user_id": str(user.id),
                "trace_id": trace_id
            })
        ...
        if login_data.device_id or login_data.app_version:
            logger.info(f"OAuth login device info", extra={
                "user_id": str(user.id),
        ...
        logger.error(f"OAuth login failed: {e}", extra={
            "provider": login_data.provider,
            "trace_id": trace_id
        })
        raise HTTPException(
...
async def oauth_logout(
    ...
    logger.info(f"OAuth logout", extra={
        "user_id": user_id,
        "device_id": device_id,
        "trace_id": trace_id
    })
...
async def revoke_token(
    ...
    try:
        ...
        logger.info(f"Token revoked", extra={
