...
class SubmissionValidator:
    ...
    def validate_api_endpoints(self) -> bool:
        ...
        endpoints = [
            ...
            ('/api/v1/search/advanced', 'Search API', {'method': 'POST', 'json': {'product': 'test'}}),
            ...
            if path.exists():
                ...
                if length > max_length:
                    self.print_result(
                        name,
                        False,
                        f"Too long: {length} chars (max {max_length})"
                    )
                    all_valid = False
                else:
                    self.print_result(
                        name,
                        True,
                        f"Length: {length}/{max_length} chars"
                    )
        ...
        try:
            ...
            if result.returncode != 0:
                self.print_result(
                    "Newman CLI",
                    False,
                    "Not installed - run: npm install -g newman"
                )
            ...
            result = subprocess.run(
                [
                    'newman', 'run',
                    str(collection_path),
                    '--env-var', f'baseUrl={self.api_url}',
                    '--reporters', 'cli,json',
